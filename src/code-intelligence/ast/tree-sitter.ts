...
import type { LanguageParser, ParsedFile, ParsedRelation, SourceRange, SymbolInfo } from "./ir.js"

type SyntaxNode = Parser.SyntaxNode

const GRAMMARS: Record<string, { language: unknown; version: string }> = {
...
function visibilityFor(language: string, text: string): string | undefined {
  if (language === "rust") return text.trimStart().startsWith("pub") ? "public" : "private"
  if (language === "java") {
  ...
  if (language === "go") return /^[A-Z]/.test(text) ? "public" : "private"
  if (language === "ruby" || language === "python") return text.trimStart().startsWith("_") ? "private" : "public"
  return undefined
}

function isPublic(language: string, name: string, text: string): boolean {
  if (language === "rust") return text.trimStart().startsWith("pub")
  if (language === "java") return /\bpublic\b/.test(text)
  if (language === "go") return /^[A-Z]/.test(name)
  return !name.startsWith("_")
}

function isTypeNode(language: string, type: string): boolean {
  if (language === "python") return type === "class_definition"
  if (language === "go") return type === "type_spec"
  if (language === "rust") return ["struct_item", "enum_item", "trait_item"].includes(type)
  if (language === "java") return ["class_declaration", "interface_declaration", "enum_declaration", "record_declaration"].includes(type)
  return ["class", "module"].includes(type)
}

function symbolKind(language: string, type: string): SymbolInfo["kind"] | undefined {
  if (isTypeNode(language, type)) {
    if (language === "ruby" && type === "module") return "module"
    if (type.includes("interface") || type === "trait_item") return "interface"
  ...
  if (type === "function_signature_item" || type === "method_elem") return "method"
  if (type === "field_declaration") return "field"
  if (type === "type_alias_declaration") return "type"
  return undefined
}

function importSource(node: SyntaxNode): string | undefined {
  const path = node.childForFieldName("path") || node.childForFieldName("module_name") || node.childForFieldName("argument")
  if (path) return path.text.replace(/^['\"]|['\"]$/g, "")
...
function isImportNode(language: string, type: string): boolean {
  if (language === "python") return ["import_statement", "import_from_statement"].includes(type)
  if (language === "go") return type === "import_spec"
  if (language === "rust") return type === "use_declaration"
  if (language === "java") return type === "import_declaration"
  return false
}

function importKind(language: string): "import" | "require" | "use" | "include" {
  if (language === "rust") return "use"
  return "import"
}

function textRange(content: string, node: SyntaxNode): SourceRange {
...
export class TreeSitterParser implements LanguageParser {
  ...
  parse(filePath: string, content: string): ParsedFile {
    ...
    parser.setLanguage(grammar.language as Parser.Language)
    parser.setTimeoutMicros(750_000)
    const tree = parser.parse(content)
    const contexts: string[] = []

    const addSymbol = (node: SyntaxNode, name: string, kind: SymbolInfo["kind"]): string => {
      ...
      if (symbol.exported) result.exports.push({ name, kind: "public", range: symbol.range })
      return qualified
    }

    const addRelation = (relation: ParsedRelation["type"], from: string, to: string, node: SyntaxNode, confidence = 0.9) => {
      if (from && to) result.relations.push({ type: relation, from, to, range: textRange(content, node), confidence })
    }

    const walk = (node: SyntaxNode): void => {
      if (node.isMissing) result.diagnostics.push({ message: `Missing syntax node: ${node.type}`, severity: "warning", range: textRange(content, node) })
      if (language === "go" && node.type === "package_clause") result.package_name = node.childForFieldName("name")?.text
      if (language === "java" && node.type === "package_declaration") result.package_name = node.namedChildren.at(-1)?.text
      ...
      if (isImportNode(language, node.type)) {
        ...
        if (source) result.imports.push({ source, names: node.namedChildren.map((child) => child.text).slice(0, 8), kind: importKind(language), range: textRange(content, node), resolution_status: "unresolved" })
      }
      ...
      if (language === "go" && node.type === "method_declaration") {
        ...
        if (receiverType) { contexts.push(receiverType.replace(/^\*/, "")); contextual = true }
      }
      if (language === "rust" && node.type === "impl_item") {
        const target = fieldText(node, "type")
        const trait = fieldText(node, "trait")
        if (target) { contexts.push(target); contextual = true }
        if (target && trait) addRelation("implements", target, trait, node)
      }
      ...
      if (kind && name) {
        ...
        if (language === "python" && node.type === "class_definition") {
          ...
          for (const child of supers?.namedChildren || []) addRelation("extends", qualified, child.text, child)
        }
        if (language === "ruby" && node.type === "class") {
          const superclass = node.childForFieldName("superclass")
          if (superclass) addRelation("extends", qualified, superclass.text, superclass)
        }
        if (language === "java" && ["class_declaration", "interface_declaration"].includes(node.type)) {
          const superclass = node.childForFieldName("superclass")
          if (superclass) addRelation("extends", qualified, superclass.text, superclass)
          const interfaces = node.childForFieldName("interfaces")
          for (const item of interfaces?.namedChildren || []) addRelation("implements", qualified, item.text, item)
        }
      }
      ...
      if (node.type.includes("call") || node.type === "call") {
        ...
        if (callee && current) {
          ...
          if (["describe", "test", "it", "specify"].includes(target)) {
            const stringNode = node.namedChildren.find((child) => child.type.includes("string"))
            if (stringNode) result.test_names.push(stringNode.text.replace(/^['\"]|['\"]$/g, ""))
          }
        }
      }

      for (const child of node.namedChildren) walk(child)
      if (pushed) contexts.pop()
      if (contextual) contexts.pop()
    }
