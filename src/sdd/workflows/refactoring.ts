...
export function createRefactoringChange(
  _graph: KnowledgeGraph,
  workflow: RefactoringWorkflow
): { change: ChangeNode; refactoring: RefactoringNode } {
  const changeId = `change-refactor-${Date.now()}`
  const refactoringId = `refactor-${Date.now()}`

  const refactoring: RefactoringNode = {
    id: refactoringId,
    type: 'refactoring',
...
export function getRefactoringInstructions(refactoring: RefactoringNode): string {
  const lines = [
