import type { KnowledgeGraph, ChangeNode, FileNode } from "../domain/types.js"
...
export interface DuplicateDetectionOptions {
  ...
  alreadyVerified?: Array<{ file_a: string; file_b: string }>
...
function detectMutantDuplicates(
  projectDir: string,
  options?: DuplicateDetectionOptions,
): MutantDuplicate[] {
  const duplicates: MutantDuplicate[] = []
  ...
  const fileData: Array<{ path: string; relPath: string; content: string; size: number; tokens: Map<string, number> }> = []
  ...
  for (const filePath of tsFiles) {
    ...
    if (excludePatterns.some(p => matchesPattern(relPath, p))) continue
...
function detectArchitectureViolations(graph: KnowledgeGraph, projectDir: string): ArchitectureViolation[] {
  ...
  const techToLayer = new Map<string, string>()
  ...
  const fileNodePathToLayer = new Map<string, string>()
 ...
 */
function inferLayerFromPath(path: string): string | null {
  const p = path.toLowerCase()
  if (p.includes("client") || p.includes("frontend") || p.includes("pages/") || p.includes("components/")) return "frontend"
  if (p.includes("server") || p.includes("backend") || p.includes("routes/") || p.includes("controllers/") || p.includes("services/") || p.includes("repositories/")) return "backend"
...
function detectPatternFragmentation(graph: KnowledgeGraph): PatternFragmentation[] {
  const fragments: PatternFragmentation[] = []
  ...
  for (const entity of entities) {
    ...
    const stemCounts: Record<string, string[]> = {}
    for (let i = 0; i < stems.length; i++) {
      const stem = stems[i]
      if (!stemCounts[stem]) stemCounts[stem] = []
...
function detectTemporalVolatility(graph: KnowledgeGraph): TemporalVolatility[] {
  const volatile: TemporalVolatility[] = []
  const changes = getNodesByType<ChangeNode>(graph, "change")

  const fileCoOccurrence: Record<string, { count: number; changeIds: string[] }> = {}
 ...
 */
function buildTokenFrequency(code: string): Map<string, number> {
 ...
 */
function tokenFrequencySimilarity(a: Map<string, number>, b: Map<string, number>): number {
...
export function formatDriftSignals(signals: DriftSignals): string {
  const lines: string[] = []
