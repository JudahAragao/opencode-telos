...
function extractFeatures(text: string): ExtractedFeature[] {
...
function extractSectionDescription(text: string, index: number): string {
...
function inferPriority(text: string, index: number): "critical" | "high" | "medium" | "low" {
  const context = text.slice(Math.max(0, index - 100), index + 200).toLowerCase()
  if (/\b(critical|essencial|obrigat|must|security|seguran|auth)\b/.test(context)) return "critical"
  if (/\b(high|importante|important|core|principal)\b/.test(context)) return "high"
  if (/\b(low|optional|opcion|nice.to.have)\b/.test(context)) return "low"
  return "medium"
}

// ─── Entity Extraction ─────────────────────────────────────────────

const ENTITY_PATTERNS: Array<{ pattern: RegExp; name: string; fields: Array<{ name: string; type: string; required?: boolean }> }> = [
  {
...
function extractEndpoints(text: string): ExtractedEndpoint[] {
  ...
  const crudPaths: Record<string, string> = {
    user: "/api/v1/users",
  ...
  while ((match = entityMentionPattern.exec(text)) !== null) {
    ...
    if (basePath && !seen.has(`GET:${basePath}`)) {
      ...
      endpoints.push(
        { method: "GET", path: basePath, description: `List ${entity}s`, relatedEntity: entity },
        { method: "GET", path: `${basePath}/:id`, description: `Get ${entity} by ID`, relatedEntity: entity },
        { method: "POST", path: basePath, description: `Create ${entity}`, relatedEntity: entity },
        { method: "PUT", path: `${basePath}/:id`, description: `Update ${entity}`, relatedEntity: entity },
        { method: "DELETE", path: `${basePath}/:id`, description: `Delete ${entity}`, relatedEntity: entity },
      )
    }
  }
...
function extractBusinessRules(text: string): ExtractedBusinessRule[] {
...
function extractArchitectureComponents(text: string): ExtractedArchitectureComponent[] {
  const components: ExtractedArchitectureComponent[] = []
  const seen = new Set<string>()

  const archPatterns: Array<{ pattern: RegExp; name: string; layer: "frontend" | "backend" | "database" | "infrastructure" | "external"; technology: string }> = [
...
function extractDecisions(text: string): ExtractedDecision[] {
...
function extractRequirements(text: string): ExtractedRequirement[] {
...
export function analyzeBriefingDeep(text: string): BriefingDeepAnalysis {
  ...
  for (const req of requirements) {
    const matchingFeature = features.find(
      (f) => f.name.toLowerCase().includes(req.name.toLowerCase().split(" ")[0]) ||
  ...
  for (const rule of businessRules) {
    const matchingFeature = features.find(
      (f) => f.name.toLowerCase().includes(rule.name.toLowerCase().split(" ")[0]) ||
  ...
  if (/\b(cms|blog|conte[uú]do|content|pagina|page)\b/i.test(text)) domains.push("cms")
  if (/\b(e-commerce|ecommerce|loja|shop|cart|carrinho)\b/i.test(text)) domains.push("ecommerce")
  if (/\b(saas|subscription|assinatura|billing)\b/i.test(text)) domains.push("saas")
  if (/\b(multi-tenant|tenant)\b/i.test(text)) domains.push("multi-tenancy")
  if (/\b(plugin|extens|addon)\b/i.test(text)) domains.push("plugin-system")
  if (/\b(theme|tema)\b/i.test(text)) domains.push("theme-system")
  if (/\b(docker|container|deploy)\b/i.test(text)) domains.push("devops")
  if (/\b(security|seguran|auth|rbac|sandbox)\b/i.test(text)) domains.push("security")
...
export function formatDeepAnalysis(analysis: BriefingDeepAnalysis): string {
  const lines = [
