 ...
 */
function isBrownfieldProject(projectDir: string): boolean {
  // Check for common source directories
  const sourceDirs = ["src", "app", "lib", "server", "api", "backend", "frontend"]
  for (const dir of sourceDirs) {
    if (existsSync(join(projectDir, dir))) return true
  }
  ...
  for (const file of configFiles) {
    if (existsSync(join(projectDir, file))) return true
  }
 ...
 */
export function generatePurposeQuestion(projectDir: string): QuestionForUser | null {
