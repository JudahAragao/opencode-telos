import type { KnowledgeGraph, SymbolNode, FileNode } from '../domain/types.js'
...
export function trackUsage(
  ...
  const symbols: SymbolUsage[] = []
  const unusedCode: UnusedCode[] = []

  const { removed, deprecated } = getExclusionSets(graph)
  const fileNodes = (graph.nodes.filter(n => n.type === 'file') as FileNode[])
    .filter(f => !isNodeExcludedOrDeprecated(f.id, f.status, removed, deprecated))
  const symbolNodes = (graph.nodes.filter(n => n.type === 'symbol') as SymbolNode[])
  ...
  for (const fileNode of fileNodes) {
    ...
    let status: FileUsage['status'] = 'used'
  ...
  for (const symbolNode of symbolNodes) {
    ...
    let status: SymbolUsage['status'] = 'used'
...
function findFileImportersWithUsage(
  filePath: string,
  graph: KnowledgeGraph,
  sourceFiles: Map<string, string>
): ImportUsage[] {
  ...
  for (const fileNode of fileNodes) {
    ...
    if (importingFilePath === filePath) continue

    const importingCode = sourceFiles.get(importingFilePath)
    if (!importingCode) continue

    const importedSymbols = extractImportsFromFile(importingCode, filePath)
    if (importedSymbols.length === 0) continue

    const actuallyUsed = importedSymbols.filter(sym => 
      isSymbolUsedInCode(sym, importingCode)
    )
    const unusedImports = importedSymbols.filter(sym => 
      !isSymbolUsedInCode(sym, importingCode)
    )
...
function findSymbolImportersWithUsage(
  symbolName: string,
  filePath: string,
  graph: KnowledgeGraph,
  sourceFiles: Map<string, string>
  ...
  for (const fileNode of fileNodes) {
    ...
    if (!importedSymbols.includes(symbolName)) continue
...
function extractImportsFromFile(sourceCode: string, targetFilePath: string): string[] {
  ...
  for (const line of lines) {
    ...
    if (!importPath.includes(targetName) && !importPath.endsWith(targetFilePath)) continue
...
function extractNamedImportsFromFile(sourceCode: string, targetFilePath: string): string[] {
  return extractImportsFromFile(sourceCode, targetFilePath)
}

function isSymbolUsedInCode(symbolName: string, code: string): boolean {
  ...
  if (foundImportLine === -1) return false
...
function findFileUsers(filePath: string, graph: KnowledgeGraph): string[] {
  const users: string[] = []
  const relationships = graph.relationships.filter(
    r => r.to === filePath && ['calls', 'instantiates', 'extends'].includes(r.type)
  )
  for (const rel of relationships) {
    const node = graph.nodes.find(n => n.id === rel.from)
    if (node) users.push(node.name)
  }
  return users
}

function findSymbolCallers(symbolName: string, filePath: string, graph: KnowledgeGraph): string[] {
  const callers: string[] = []
  const symbolNode = graph.nodes.find(
    n => n.type === 'symbol' && n.name === symbolName && (n as SymbolNode).metadata.file_path === filePath
  )
  if (!symbolNode) return callers

  const relationships = graph.relationships.filter(
    r => r.to === symbolNode.id && ['calls', 'invokes'].includes(r.type)
  )
  for (const rel of relationships) {
    const node = graph.nodes.find(n => n.id === rel.from)
    if (node) callers.push(node.name)
  }
  return callers
}

function findSymbolReferences(symbolName: string, filePath: string, graph: KnowledgeGraph): string[] {
  const references: string[] = []
  const symbolNode = graph.nodes.find(
    n => n.type === 'symbol' && n.name === symbolName && (n as SymbolNode).metadata.file_path === filePath
  )
  if (!symbolNode) return references

  const relationships = graph.relationships.filter(
    r => r.to === symbolNode.id && ['references', 'accesses'].includes(r.type)
  )
  for (const rel of relationships) {
    const node = graph.nodes.find(n => n.id === rel.from)
    if (node) references.push(node.name)
  }
  return references
}

function findSpecConnections(nodeId: string, graph: KnowledgeGraph): string[] {
  const connectedSpecs: string[] = []
...
function detectEntryPoint(filePath: string, sourceCode: string): boolean {
  if (filePath.endsWith('main.ts') || filePath.endsWith('main.js')) return true
  if (filePath.endsWith('index.ts') || filePath.endsWith('index.js')) return true
  if (filePath.includes('app.ts') || filePath.includes('app.js')) return true
  if (filePath.includes('server.ts') || filePath.includes('server.js')) return true
  if (sourceCode.includes('createApp') || sourceCode.includes('createServer')) return true
  if (sourceCode.includes('bootstrap') || sourceCode.includes('main()')) return true
  return false
}
