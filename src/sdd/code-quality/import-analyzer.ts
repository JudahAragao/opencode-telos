...
export interface ImportAnalysisOptions {
  ...
  fileCache?: Map<string, { analysis: ImportAnalysis; contentHash: string }>
  /** Hash of the source content for cache invalidation. */
  contentHash?: string
}

export function analyzeImports(sourceCode: string, filePath: string, options?: ImportAnalysisOptions): ImportAnalysis {
...
function filterImports(imports: ImportInfo[], options?: ImportAnalysisOptions): ImportInfo[] {
  if (!options?.focusModules?.length && !options?.excludeModules?.length) return imports

  return imports.filter((imp) => {
    if (options.excludeModules?.length) {
      if (options.excludeModules.some((m) => imp.source.includes(m))) return false
    }
