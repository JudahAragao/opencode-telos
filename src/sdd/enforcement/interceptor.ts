...
export interface EnforcementOptions {
  ...
  validationCache?: Map<string, { valid: boolean; errors: any[]; timestamp: number }>
}
...
export function classifyChangeRequest(description: string): ChangeRequest {
...
export function enforceSddFirst(
  ...
  if (approvalLevel !== "BLOCKED" && !options?.forceApprovalLevel && request.affected_files && request.affected_files.length > 0) {
    ...
    if (crossLayerDetected) {
      if (approvalLevel === "AUTO") approvalLevel = "REVIEW"
      else if (approvalLevel === "REVIEW") approvalLevel = "APPROVAL"
    }
  }
  ...
  if (options?.skipValidation) {
    ...
    if (!validation.valid) {
      ...
      result.blocking_reasons = validation.errors.map((e: any) => `[${e.code}] ${e.message}`)
  ...
  if (approvalLevel === "APPROVAL") {
    ...
    if (!hasTestCoverage && affectedReqs.length > 0) {
      ...
      result.blocking_reasons.push(
        `APPROVAL-level change affects ${affectedReqs.length} entity(ies) but none have test coverage. ` +
  ...
  if (!options?.skipImpactAnalysis && request.affected_entities && request.affected_entities.length > 0) {
    ...
    for (const entityId of request.affected_entities) {
      ...
      for (const n of impact.direct) impactNodes.add(n.id)
      for (const n of impact.indirect) impactNodes.add(n.id)
    }
 ...
 */
function checkTestCoverageForAffected(
  graph: KnowledgeGraph,
  affectedEntities: string[],
): boolean {
  if (affectedEntities.length === 0) return true

  for (const entityName of affectedEntities) {
    // Find entity node by name or ID
    const entity = graph.nodes.find(
      (n) => (n.type === "entity" || n.type === "requirement" || n.type === "feature") &&
    ...
    const hasTest = graph.relationships.some(
      (r) => r.from === entity.id && r.type === "tested_by"
    )
...
export function buildEnforcementPrompt(
  request: ChangeRequest,
 ...
 */
export function enforceSmartBatch(
  graph: KnowledgeGraph,
  request: ChangeRequest,
  affectedEntities: string[] = [],
  affectedFiles: string[] = [],
  options?: EnforcementOptions,
): EnforcementResult & { auto_completed: boolean } {
...
export function getSddEnforcementRules(): string {
  return `
