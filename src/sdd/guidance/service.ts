...
export function applyGuidancePatch(
  ...
  const working = structuredClone(graph) as KnowledgeGraph
  ...
  const applyOne = (nodeToUpdate: AnyNode, patchToApply: Record<string, unknown>): AnyNode => {
    ...
    if (patchToApply.metadata && typeof patchToApply.metadata === "object" && !Array.isArray(patchToApply.metadata)) {
      const metadataPatch = patchToApply.metadata as Record<string, unknown>
      ...
      if (protectedKey) throw new Error(`Guidance cannot directly modify protected metadata field ${protectedKey}`)
      updates.metadata = {
    ...
    return updateNode(working, nodeToUpdate.id, { ...updates, created_by: actor } as Partial<AnyNode>)
  }
  ...
  for (const update of propagated) {
    if (!hasSupportedPatch(getNode(working, update.node_id) as AnyNode, update.patch)) throw new Error(`Guidance proposal contains no supported updates for ${update.node_id}`)
  }
  if (target.type === "acceptance_criterion") {
    const updatedTarget = applyOne(target, proposal)
    for (const update of propagated) applyOne(getNode(working, update.node_id) as AnyNode, update.patch)
