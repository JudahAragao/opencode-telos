...
export function validateAgainstConstitution(
  ...
  options?: ConstitutionValidationOptions,
): ConstitutionResult {
  ...
  let allPrinciples = constitutions.flatMap((c) =>
    (c.metadata.principles || []).map((p) => ({
      ...p,
  ...
  let nodesToCheck = graph.nodes.filter(n =>
    n.type !== "constitution" && n.type !== "project" && !excludeNodeSet.has(n.id)
  )
  ...
  for (const node of nodesToCheck) {
    ...
    for (const principle of allPrinciples) {
      ...
      if (principleScopeIndex) {
        ...
        if (applicableTypes && !applicableTypes.has(node.type)) continue
      }

      const scopeRegex = principle.scope ? new RegExp(principle.scope, "i") : null
      if (scopeRegex && !scopeRegex.test(nodeStr)) continue

      const cacheKey = `${node.id}||${principle.id}`
      let violation: ConstitutionViolation | null = null
 ...
 */
function buildPrincipleScopeIndex(
  principles: Array<{ id: string; scope?: string }>,
  ...
  for (const p of principles) {
    ...
    if (/entity|entidade/i.test(p.scope)) types.add("entity")
    if (/endpoint|rota|api/i.test(p.scope)) types.add("endpoint")
    if (/feature|funcionalidade/i.test(p.scope)) types.add("feature")
    if (/requirement|requisito/i.test(p.scope)) types.add("requirement")
    if (/file|arquivo/i.test(p.scope)) types.add("file")
    if (/architecture|arquitetura/i.test(p.scope)) types.add("architecture_component")
    if (/rule|regra/i.test(p.scope)) types.add("business_rule")
    if (/task|tarefa/i.test(p.scope)) types.add("task")
    if (/test|teste/i.test(p.scope)) types.add("test")
    if (types.size > 0) index.set(p.id, types)
  }
  return index
}

function checkPrinciple(
  principle: { id: string; rule: string; severity: "must" | "should" | "may"; scope?: string },
  node: AnyNode,
...
function extractForbiddenPattern(rule: string): string | null {
  const neverMatch = rule.match(/never\s+(?:use|include|contain|allow)\s+(\S+)/i)
  if (neverMatch) return neverMatch[1].toLowerCase()

  const nuncaMatch = rule.match(/nunca\s+(?:usar|incluir|conter|permitir)\s+(\S+)/i)
  if (nuncaMatch) return nuncaMatch[1].toLowerCase()

  return null
}

function extractRequiredPattern(rule: string): string | null {
  const alwaysMatch = rule.match(/always\s+(?:use|include|contain)\s+(\S+)/i)
  if (alwaysMatch) return alwaysMatch[1].toLowerCase()

  const sempreMatch = rule.match(/sempre\s+(?:usar|incluir|conter)\s+(\S+)/i)
  if (sempreMatch) return sempreMatch[1].toLowerCase()

  return null
}

export function formatConstitutionResult(result: ConstitutionResult): string {
