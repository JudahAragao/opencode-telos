    ...
    getTools() {
      return [
        ...
        },
        {
          ...
          inputSchema: { type: "object" as const, properties: { criterion_id: { type: "string" }, actor: { type: "string" }, observation: { type: "string" } }, required: ["criterion_id", "observation"] },
        },
        {
        ...
        },
        {
          ...
          inputSchema: { type: "object" as const, properties: { requirement_id: { type: "string" }, actor: { type: "string" }, observation: { type: "string" } }, required: ["requirement_id"] },
        },
        {
          name: "sdd_create_criterion",
          description: "Create and link an acceptance criterion to a requirement",
          inputSchema: { type: "object" as const, properties: { requirement_id: { type: "string" }, text: { type: "string" }, actor: { type: "string" } }, required: ["requirement_id", "text"] },
        },
        {
          name: "sdd_update_criterion",
          description: "Update criterion text, incrementing its version and invalidating acceptance",
          inputSchema: { type: "object" as const, properties: { criterion_id: { type: "string" }, text: { type: "string" }, actor: { type: "string" }, observation: { type: "string" } }, required: ["criterion_id", "text"] },
        },
        {
          name: "sdd_migrate_acceptance",
          description: "Materialize legacy acceptance metadata into graph nodes",
          inputSchema: { type: "object" as const, properties: { actor: { type: "string" } } },
        },
        {
        ...
        },
        {
          ...
          inputSchema: { type: "object" as const, properties: { change_id: { type: "string" }, actor: { type: "string" }, observation: { type: "string" }, evidence: { type: "array" } }, required: ["change_id"] },
        },
        {
    ...
    async handleToolCall(toolName: string, _args: Record<string, unknown>) {
      ...
      switch (toolName) {
        ...
        case "sdd_reopen_criterion": {
          ...
          if (!checkPermission(getUserRoleWithAuth(projectDir, actor), permission as any, projectDir)) return { error: `Permission denied: ${permission}` }
          const service = new AcceptanceService(graph, true, createAcceptanceAuditSink(projectDir))
        ...
        case "sdd_accept_all": {
          ...
          if (result.failed.length > 0) return { error: JSON.stringify(result) }
          repo.saveGraph(graph)
          return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] }
        }
        case "sdd_create_criterion": {
          ...
          const actor = String(_args.actor || process.env.USER || "mcp")
          if (!checkPermission(getUserRoleWithAuth(projectDir, actor), "create_requirement", projectDir)) return { error: "Permission denied: create_requirement" }
          const service = new AcceptanceService(graph, true, createAcceptanceAuditSink(projectDir))
        ...
        case "sdd_reject_final": {
          ...
          const result = transitionFinalAcceptance(graph, changeId, toolName === "sdd_accept_final" ? "ACCEPTED" : "REJECTED", {
            ...
            evidence: Array.isArray(_args.evidence) ? _args.evidence as Array<Record<string, unknown>> : undefined,
          })
          repo.saveGraph(graph)
