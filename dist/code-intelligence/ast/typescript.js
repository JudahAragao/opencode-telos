...
function symbolRange(node, source) {
...
export class TypeScriptParser {
    ...
    parse(filePath, content) {
        ...
        const symbolStack = [];
        const addImport = (item) => result.imports.push(item);
        const addExport = (item) => result.exports.push(item);
        const addSymbol = (node, name, kind, exported = false, signature) => {
            ...
            if (exported)
                addExport({ name, kind: "named", range: symbolRange(node, source) });
        ...
        const visit = (node) => {
            ...
            if (ts.isExportDeclaration(node)) {
                ...
                if (node.exportClause && ts.isNamedExports(node.exportClause)) {
                    for (const element of node.exportClause.elements)
                        addExport({ name: element.name.text, source_name: element.propertyName?.text, source: sourceName, kind: sourceName ? "reexport" : "named", range: symbolRange(element, source) });
                }
                else if (sourceName)
                    addExport({ name: "*", source: sourceName, kind: "reexport", range: symbolRange(node, source) });
            }
            ...
            if (ts.isCallExpression(node)) {
                ...
                if (["describe", "test", "it", "specify"].includes(callee) && first && ts.isStringLiteral(first))
                    result.test_names.push(first.text);
                const current = symbolStack.at(-1);
                if (current && callee)
                    result.relations.push({ type: "calls", from: current, to: callee, range: symbolRange(node, source), confidence: 0.8 });
            }
