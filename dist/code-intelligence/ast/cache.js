...
export function loadAstCache(projectDir) {
    ...
    try {
        ...
        const entries = Object.entries(document.entries).filter(([, entry]) => Boolean(entry && entry.content_hash && entry.parser && entry.parser_version && entry.parsed));
...
export function parseWithCache(projectDir, filePath, content, cache) {
...
export function saveAstCache(projectDir, cache) {
    if (!cache.dirty)
        return;
    ...
    try {
        ...
        atomicWriteFile(cachePath(projectDir), JSON.stringify({ version: 2, entries }));
    }
    finally {
