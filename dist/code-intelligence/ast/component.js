...
export class ComponentParser {
    name = "component-script-typescript";
    version = "1";
    scriptParser = new TypeScriptParser();
    supports(language) { return language === "vue" || language === "svelte"; }
    parse(filePath, content) {
        ...
        for (const match of blocks) {
            ...
            for (let index = 0; index < script.length; index++)
                masked[start + index] = script[index];
        }
        ...
        if (blocks.length === 0)
            parsed.diagnostics.push({ message: "No script block found in component", severity: "info" });
        return parsed;
    }
}
