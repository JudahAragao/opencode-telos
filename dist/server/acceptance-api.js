...
export function handleAcceptanceMigration(projectDir, raw = {}) {
    const parsed = z.object({ actor: z.string().min(1).max(200).optional() }).safeParse(raw ?? {});
    if (!parsed.success)
        return { status: 400, body: { error: "Invalid migration payload", issues: parsed.error.issues } };
    try {
        const repo = createRepository(projectDir);
        if (!repo.isInitialized())
            return { status: 503, body: { error: "SDD not initialized" } };
...
export function handleFinalAcceptance(projectDir, changeId, status, raw) {
    const parsed = mutationSchema.safeParse(raw ?? {});
    if (!parsed.success)
        return { status: 400, body: { error: "Invalid final acceptance payload", issues: parsed.error.issues } };
    try {
        const repo = createRepository(projectDir);
        if (!repo.isInitialized())
            return { status: 503, body: { error: "SDD not initialized" } };
        const graph = repo.loadGraph();
        const currentActor = actor(parsed.data.actor);
        const permission = status === "ACCEPTED" ? "accept_final" : "reject_final";
