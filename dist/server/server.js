...
import { handleAcceptAll, handleAcceptanceMigration, handleAcceptanceMutation, handleGuidance, handleImpact, handleListGuidance, handleListAcceptance, handleFinalAcceptance, } from "./acceptance-api.js";
    ...
    async handleRequest(url, req) {
        ...
        try {
            ...
            if (path === "/api/acceptance/accept-all" && req.method === "POST") {
                ...
                if (!requirementId)
                    return this.jsonResponse({ error: "requirement_id is required" }, corsHeaders, 400);
            ...
            if (path.startsWith("/api/acceptance/final/") && req.method === "POST") {
                ...
                if (!changeId || !action)
                    return this.jsonResponse({ error: "Change id and final action are required" }, corsHeaders, 400);
            ...
            if (path.startsWith("/api/acceptance/")) {
                ...
                const action = rest[1];
                if (!criterionId || !action || req.method !== "POST")
                    return this.jsonResponse({ error: "Criterion id and action are required" }, corsHeaders, 400);
            ...
            if (path === "/api/guidance" && req.method === "POST") {
                ...
                if (!targetId)
                    return this.jsonResponse({ error: "target_node_id is required" }, corsHeaders, 400);
            ...
            if (path.startsWith("/api/guidance/")) {
                ...
                const action = rest[1];
                if (!guidanceId || !action || req.method !== "POST")
                    return this.jsonResponse({ error: "Guidance id and action are required" }, corsHeaders, 400);
