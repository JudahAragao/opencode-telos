...
import { checkPermission, getUserRoleWithAuth, getAuditLog, formatAuditLog, loadPermissions, setRole, canBootstrapPermissions, getRequiredApprovals, } from "../../sdd/permissions/access.js";
...
import { savePermissionsConfigHandler, listSnapshotsHandler, verifyUsageHandler, findDeadCodeHandler, removeDeadCodeHandler, planImplementationHandler, analyzeCodebaseHandler, createMigrationHandler, createExperimentHandler, createFlagHandler, createTenantHandler, generateDashboardHandler, reportIncidentHandler, createSlaHandler, estimateCostHandler, knowledgeTransferHandler, disasterRecoveryPlanHandler, configDriftHandler, workflowExportHandler, } from "../tool-handlers.js";
...
function nodeMutationPermission(type) {
    if (type === "feature")
        return "create_feature";
    if (type === "requirement" || type === "acceptance_criterion")
        return "create_requirement";
    if (type === "entity" || type === "value_object")
        return "create_entity";
    if (type === "endpoint" || type === "api")
...
export function createGraphMutationTool() {
    return tool({
        description: "Modificar a estrutura do Knowledge Graph. " +
        ...
        async execute(args, ctx) {
            ...
            switch (args.action) {
                case "add_node": {
                    ...
                    if (!checkPermission(getUserRoleWithAuth(ctx.directory, actor), permission, ctx.directory))
                    ...
                    if (args.metadata_json) {
                        try {
                            ...
                            return "Invalid JSON in metadata_json";
                        }
                    }
                    const nodeType = args.type;
                    if (nodeType === "business_rule" && !metadata.rule_text)
                        metadata.rule_text = args.description_text || args.name || "";
                    ...
                    const node = {
                        id: nodeId, type: args.type, name: args.name,
                    ...
                    if (args.parent_id) {
                        try {
                            ...
                            sddDebug("composite", `Failed to add parent relationship for ${nodeId}`);
                        }
                    }
                ...
                case "update_node": {
                    if (!args.node_id || !args.updates_json)
                        return "node_id and updates_json are required";
                    let updates;
                    try {
                        updates = JSON.parse(args.updates_json);
                    }
                    catch {
                        return "Invalid JSON in updates_json";
                    }
                ...
                case "add_relationship": {
                    ...
                    const actor = process.env.USER || process.env.USERNAME || "current";
                    if (!checkPermission(getUserRoleWithAuth(ctx.directory, actor), "create_change", ctx.directory))
                        return "Permission denied: create_change";
                    try {
        ...
        async execute(args, ctx) {
            ...
            switch (args.action) {
                ...
                case "list_nodes": {
                    ...
                    if (nodes.length === 0)
                        return `No nodes of type "${args.type}".`;
...
export function createTraverseTool() {
    return tool({
        ...
        async execute(args, ctx) {
            ...
            switch (args.action) {
                ...
                case "subgraph": {
                    ...
                    for (const n of sub.nodes.slice(0, 20))
                        lines.push(`- **${n.id}** (${n.type}): ${n.name}`);
                    return lines.join("\n");
                }
                case "find_path": {
                    if (!args.to_id)
                        return "to_id is required for find_path";
                    const path = findPath(graph, args.node_id, args.to_id);
                    if (!path)
                        return `No path found between ${args.node_id} and ${args.to_id}.`;
...
export function createPermissionsTool() {
    return tool({
            ...
            switch (args.action) {
                case "set_role": {
                    if (!args.user || !args.role)
                        return "user and role are required";
                ...
                case "check": {
                    if (!args.user || !args.permission)
                        return "user and permission are required";
                    const role = getUserRoleWithAuth(ctx.directory, args.user);
                    const has = checkPermission(role, args.permission, ctx.directory);
                ...
                case "save_config": {
                    if (!args.config_json)
                        return "config_json is required";
    ...
    return tool({
        ...
        async execute(args, ctx) {
            switch (args.action) {
                ...
                case "conflicts": {
                    if (!args.remote_graph_path)
                        return "remote_graph_path is required";
                    const repo = getRepo(ctx.directory);
                    if (!repo.isInitialized())
                        return "SDD not initialized.";
                ...
                case "merge": {
                    ...
                    const remote = JSON.parse((await import("fs")).readFileSync(projectPath(ctx.directory, args.remote_graph_path), "utf-8"));
...
export function createGraphAdminTool() {
    return tool({
            ...
            switch (args.action) {
                ...
                case "learn": {
                    ...
                    if (subAction === "show") {
                        ...
                        if (!patterns)
                            return "No patterns learned yet. Run with learn_action=learn first.";
                        return formatPatterns(patterns);
                    }
                    if (subAction === "suggest") {
                        if (!args.node_type)
                            return "node_type is required for suggest";
                        const patterns = loadPatterns(ctx.directory);
                        if (!patterns)
                            return "No patterns learned yet. Run with learn_action=learn first.";
...
export function createEnterpriseTool() {
    return tool({
...
export function createDriftWhitelistTool() {
    return tool({
        ...
        async execute(args, ctx) {
            switch (args.action) {
                case "add": {
                    if (!args.file_path || !args.reason)
                        return "file_path and reason are required";
