...
import { detectBrownfieldFindings, findingFingerprint, formatFindingsReport, getFindings, resolveFinding, transitionFinding, upsertFinding, createFindingTask } from "../sdd/brownfield/findings.js";
import { generateCicd, writeCicdFiles, formatCicdResults } from "../sdd/cicd/generators.js";
import { addAuditEntry, checkPermission, detectRemote, formatRemoteStatus, getUserRoleWithAuth } from "../sdd/permissions/access.js";
...
function requireApprovedWorkflowChange(directory, sessionId, graph) {
    const state = getWorkflowState(workflowScope(directory, sessionId));
    const candidateId = state.changeId;
    const change = candidateId
        ? graph.nodes.find((node) => node.id === candidateId && node.type === "change")
        : undefined;
    if (change?.status === "APPROVED")
        return change;
    return graph.nodes.find((node) => node.type === "change" && node.status === "APPROVED");
}
...
function ensureTargetRequirementForFinding(graph, finding) {
    const existing = graph.nodes.find((node) => node.type === "requirement" && node.metadata.source_finding_id === finding.id);
    if (existing)
        return existing;
    ...
    try {
        addRelationship(graph, requirement.id, finding.id, "derived_from", { source: "reverse_engineering" });
    }
    catch { }
    return requirement;
}
function materializeBrownfieldFindings(graph, inputs, purpose) {
    let created = 0;
    let tasks = 0;
    let resolvedForTarget = 0;
    for (const input of inputs) {
        const fingerprint = input.fingerprint ?? findingFingerprint(input);
        const wasExisting = graph.nodes.some((node) => node.type === "finding" && node.metadata.fingerprint === fingerprint);
...
function attachResponseCache(tools) {
        ...
        definition.execute = async (args, ctx) => {
            ...
            if (typeof response === "string" && repo?.isInitialized()) {
                try {
                    ...
                    sddDebug("tools", `Failed to cache response for ${toolName}`);
                }
            }
            return response;
        };
...
function completionGateFailures(projectDir, graph, changeId) {
    ...
    if (config.acceptance.enabled && config.acceptance.require_before_change_completion) {
        ...
        if (!acceptance.allowed)
            failures.push(acceptance.reason);
    }
    if (config.acceptance.enabled && config.acceptance.require_final_acceptance_before_completion) {
        ...
        if (change && getFinalAcceptanceStatus(change) !== "ACCEPTED")
            failures.push("Final delivery acceptance is still pending or rejected.");
    }
    return failures;
}
/**
 ...
 */
export function invalidateCacheForMutation(directory, nodeTypes, relTypes = []) {
    try {
        const cacheMgr = getCacheManager(directory);
        cacheMgr.invalidatePartial(nodeTypes, relTypes);
    }
    catch (error) {
        sddDebug("tools", "Failed to invalidate cache after mutation");
    }
}
    ...
    const tools = {
        "sdd.acceptance": tool({
            ...
            async execute(args, ctx) {
                ...
                const evidence = args.evidence_json ? (() => { try {
                    return JSON.parse(args.evidence_json);
                }
                catch {
                    return undefined;
                } })() : undefined;
                ...
                try {
                    const requiredPermission = args.action === "create" || args.action === "update_text" || args.action === "migrate"
                        ? "create_requirement"
                    ...
                    if (args.action === "list" || args.action === "summary") {
                        if (!args.requirement_id)
                            return "requirement_id is required";
                        if (args.action === "summary")
                            return JSON.stringify(service.summary(args.requirement_id), null, 2);
                    ...
                    if (args.action === "create") {
                        if (!args.requirement_id || !args.text)
                            return "requirement_id and text are required";
                    ...
                    if (args.action === "update_text") {
                        if (!args.criterion_id || !args.text)
                            return "criterion_id and text are required";
        ...
        "sdd.impact": tool({
            ...
            async execute(args, ctx) {
                ...
                if (!repo.isInitialized())
                    return "SDD not initialized.";
                try {
                    return formatNodeImpact(analyzeNodeImpact(repo.loadGraph(), args.node_id, args.depth || 5));
                }
                catch (error) {
                    return `Error: ${error instanceof Error ? error.message : String(error)}`;
                }
        ...
        "sdd.node_guidance": tool({
            ...
            async execute(args, ctx) {
                ...
                const actor = process.env.USER || process.env.USERNAME || "current";
                try {
                    const requiredPermission = args.action === "apply" ? "apply_node_guidance" : "guide_node";
                    ...
                    if (args.action === "create") {
                        if (!args.node_id || !args.instruction)
                            return "node_id and instruction are required";
                    ...
                    if (!args.guidance_id)
                        return "guidance_id is required";
                    ...
                    if (!args.proposal_json && (args.action === "propose" || args.action === "apply"))
                        return "proposal_json is required";
                    const proposal = args.proposal_json ? JSON.parse(args.proposal_json) : {};
        ...
        "sdd.inspect": tool({
            description: "Inspect the current state of the SDD Knowledge Graph. Shows stats, node counts by type, " +
        ...
        "sdd.query_graph": tool({
            description: "Query the SDD Knowledge Graph. Search for nodes by text, type, or status. " +
            ...
            async execute(args, ctx) {
                ...
                if (exact) {
                    ...
                    if (exact.description)
                        lines.push(`**Description:** ${exact.description}`);
                ...
                let results = searchNodesIndexed(indices, args.query, args.type);
                ...
                if (queryWords.length > 1 && results.length > 20) {
                    ...
                    if (andResults.length > 0 && andResults.length < results.length) {
                        results = andResults
                            ...
                            .filter((n) => n !== undefined && (!args.type || n.type === args.type));
                    }
                }
                if (results.length === 0)
        ...
        "sdd.analyze_impact": tool({
            description: "Perform impact analysis for a change request. Traverses the Knowledge Graph " +
            ...
            async execute(args, ctx) {
                ...
                if (impact.direct.length === 0) {
                    ...
                    if (impact.direct.length > 15)
                        lines.push(`- ... and ${impact.direct.length - 15} more`);
                }
                lines.push("\n### Indirect Impact");
                if (impact.indirect.length === 0) {
                    ...
                    if (impact.indirect.length > 15)
                        lines.push(`- ... and ${impact.indirect.length - 15} more`);
                }
                // Concrete actions
                if (actions.length > 0) {
                    lines.push("\n### 🎯 Recommended Actions");
        ...
        "sdd.create_change": tool({
            ...
            async execute(args, ctx) {
                ...
                if (args.new_nodes_json) {
                    try {
                        newNodes = JSON.parse(args.new_nodes_json);
                    }
                    catch {
                        return "Invalid JSON in new_nodes_json";
                    }
                }
                let modifiedNodes = [];
                if (args.modified_nodes_json) {
                    try {
                        modifiedNodes = JSON.parse(args.modified_nodes_json);
                    }
                    catch {
        ...
        "sdd.discover": tool({
            description: "Analyze a user briefing and return structured discovery questions for missing information. " +
        ...
        "sdd.update_from_answers": tool({
            description: "Update the Knowledge Graph based on user answers to discovery questions. " +
        ...
        "sdd.validate": tool({
            description: "Validate the SDD Knowledge Graph for structural integrity, semantic correctness, " +
        ...
        "sdd.detect_drift": tool({
            description: "Detect SDD drift - find discrepancies between the specification and the codebase.",
            args: {},
            async execute(_args, ctx) {
                const repo = getRepo(ctx.directory);
                if (!repo.isInitialized())
        ...
        "sdd.check_migrations": tool({
            description: "Check if SDD data needs migration. Analyzes graph.yaml vs graph.db sync, " +
            ...
            async execute(args, ctx) {
                ...
                if (!consistency.consistent)
                    issues.push(consistency.reason);
        ...
        "sdd.run_migrations": tool({
            description: "Execute all pending SDD migrations to fix data issues.",
            args: {},
            async execute(_args, ctx) {
        ...
        "sdd.get_context": tool({
            description: "Build a context pack for a specific node. Returns all relevant nodes, " +
            ...
            async execute(args, ctx) {
                ...
                const subgraphRels = graph.relationships.filter(r => subgraphNodeIds.has(r.from) && subgraphNodeIds.has(r.to));
                // Also include cross-boundary relationships for context
                const boundaryRels = graph.relationships.filter(r => (subgraphNodeIds.has(r.from) && !subgraphNodeIds.has(r.to)) ||
                    (!subgraphNodeIds.has(r.from) && subgraphNodeIds.has(r.to))).slice(0, 10);
                const lines = [
                ...
                if (subgraphRels.length > 0) {
                    ...
                    if (subgraphRels.length > 20)
                        lines.push(`- ... and ${subgraphRels.length - 20} more`);
                }
        ...
        "sdd.approve_change": tool({
            ...
            async execute(args, ctx) {
                ...
                if (changeNode) {
                    const creator = changeNode.created_by || changeNode.metadata?.created_by;
                    const approver = process.env.USER || process.env.USERNAME || "current";
                    if (creator && creator === approver) {
                        addAuditEntry(ctx.directory, approver, "sdd.approve_change", args.change_id, "denied", "Self-approval prevented");
                        return [
        ...
        "sdd.change_history": tool({
            ...
            async execute(args, ctx) {
                ...
                if (history.length === 0)
                    return "No changes found in history.";
        ...
        "sdd.pending_changes": tool({
            ...
            async execute(_args, ctx) {
                ...
                const changeLines = pending.map(change => `- **${change.id}:** ${change.name} [${change.status}] (approval: ${change.metadata.approval_level})`);
                lines.push(...truncateList(changeLines, 15));
                // Detect stale changes
                const now = Date.now();
                const staleThreshold = 7 * 24 * 60 * 60 * 1000;
                const staleChanges = pending.filter((c) => now - new Date(c.created_at).getTime() > staleThreshold);
        ...
        "sdd.generate_code": tool({
            description: "Generate project code from the SDD Knowledge Graph. Creates actual project files " +
            ...
            async execute(args, ctx) {
                ...
                const approvedChanges = graph.nodes.filter((n) => n.type === "change" && n.status === "APPROVED");
                ...
                if (result.backups.length > 0)
                    lines.push(`- Backups created: ${result.backups.length}`);
        ...
        "sdd.enforce": tool({
            description: "Enforce SDD-first workflow. Classifies a user request, creates a Change node, " +
            ...
            async execute(args, ctx) {
                ...
                if (graph.metadata.purpose === "reverse_engineering") {
                    const blockers = graph.nodes.filter((node) => node.type === "finding" &&
                        ["critical", "high"].includes(String(node.metadata.severity)) &&
        ...
        "sdd.enforce_rules": tool({
            description: "Returns the SDD enforcement rules. Use this to understand the mandatory workflow.",
            args: {},
            async execute() {
        ...
        "sdd.full_cycle": tool({
            description: "Execute a complete SDD change cycle: analyze request → update spec → validate → " +
            ...
            async execute(args, ctx) {
                ...
                if (!enforcement.allowed) {
                    ...
                    if (enforcement.blocking_reasons) {
                        for (const r of enforcement.blocking_reasons)
                            lines.push(`- ${r}`);
                    }
                    repo.saveGraph(graph);
                    invalidateCacheForMutation(ctx.directory, ["change"], []);
                ...
                if (enforcement.change_id)
                    markEnforced(enforcement.change_id, workflowScope(ctx.directory, ctx.sessionID));
                ...
                if (enforcement.change_id)
                    saveExecutableValidation(ctx.directory, enforcement.change_id, execution);
                ...
                if (enforcement.change_id) {
                    ...
                    try {
                        if (!execution.passed || !execution.verified || !execution.functional_verified) {
                            ...
                            for (const gap of execution.functional_gaps || [])
                                lines.push(`- Functional evidence: ${gap}`);
                            repo.saveGraph(graph);
                            return lines.join("\n");
                        }
                        const { completeChangeWithPromiseCheck } = await import("../sdd/changes/manager.js");
        ...
        "sdd.toggle": tool({
            description: "Enable or disable SDD enforcement. When disabled, code changes can be made freely " +
            ...
            async execute(args, ctx) {
                ...
                const dir = ctx.projectDir || ctx.directory;
        ...
        "sdd.constitution": tool({
            description: "View or update the project constitution. The constitution defines high-level principles " +
            ...
            async execute(args, ctx) {
                ...
                if (args.action === "add") {
                    ...
                    const existing = graph.nodes.find((n) => n.type === "constitution");
                    const now = new Date().toISOString();
                    if (existing) {
                ...
                if (args.action === "remove") {
                    if (!args.principle_id)
                        return "Error: 'principle_id' is required for remove action.";
                    const existing = graph.nodes.find((n) => n.type === "constitution");
                    if (!existing)
                    ...
                    if (idx === -1)
                        return `Principle ${args.principle_id} not found.`;
        ...
        "sdd.promises": tool({
            description: "View and manage specification promises. Promises are verifiable assertions extracted " +
            ...
            async execute(args, ctx) {
                ...
                if (args.action === "list") {
                    ...
                    if (promises.length === 0)
                        return "No promises found. Add requirements with acceptance_criteria or business rules.";
        ...
        "sdd.quality": tool({
            description: "Calculate and display the project quality score (0-100%) with trend analysis. " +
        ...
        "sdd.session_handoff": tool({
            description: "Generate a session handoff pack. Shows the current SDD state including active changes, " +
        ...
        "sdd.anti_patterns": tool({
            description: "Detect anti-patterns in the spec graph: god nodes, circular dependencies, " +
        ...
        "sdd.clone_detection": tool({
            description: "Detect code clones (duplicate code blocks) across the project source files. " +
        ...
        "sdd.contradictions": tool({
            description: "Detect contradictions in the spec graph: conflicting requirements, " +
        ...
        "sdd.coverage": tool({
            description: "Calculate test coverage for requirements. Shows which requirements " +
        ...
        "sdd.install_hooks": tool({
            description: "Install Git shell hooks for SDD integration (pre-commit, post-checkout, post-merge).",
            args: {
            ...
            async execute(args, ctx) {
                ...
                if (!change)
                    return "An approved Change is required before installing hooks.";
                const hooks = args.hooks || ["pre-commit"];
        ...
        "sdd.brownfield_scan": tool({
            description: "Scan an existing (brownfield) project to understand its structure, " +
        ...
        "sdd.findings": tool({
            description: "Gerenciar descobertas brownfield persistentes. No modo documentation, mantém problemas no AS-IS e cria tasks de correção; " +
            ...
            async execute(args, ctx) {
                const repo = getRepo(ctx.directory);
                if (!repo.isInitialized())
                    return "SDD not initialized. Run sdd.reverse_engineer first.";
                const graph = repo.loadGraph();
                const action = args.action;
                const purpose = args.purpose ?? graph.metadata.purpose ?? "documentation";
                if (action === "scan") {
                    const brownfield = scanExistingProject(ctx.directory);
                    const scan = detectBrownfieldFindings(ctx.directory, brownfield, purpose);
                    const summary = materializeBrownfieldFindings(graph, scan.findings, purpose);
                    repo.saveGraph(graph);
                    ...
                    return [
                        ...
                        "",
                        formatFindingsReport(graph, purpose),
                    ].join("\n");
                }
                if (action === "list" || action === "report" || action === "readiness") {
                    ...
                    if (action === "list")
                        return JSON.stringify(findings.map((finding) => ({ id: finding.id, title: finding.name, status: finding.status, severity: finding.metadata.severity, category: finding.metadata.category })), null, 2);
                    if (action === "readiness") {
                        const blockers = findings.filter((finding) => !["resolved", "closed", "accepted", "wont_fix"].includes(finding.status) && ["critical", "high"].includes(finding.metadata.severity));
                        return [
                    ...
                    return formatFindingsReport(graph, purpose);
                }
                if (!args.finding_id)
                    return "`finding_id` is required for this action.";
                ...
                if (action === "transition") {
                    if (!args.status)
                        return "`status` is required for transition.";
                    transitionFinding(graph, args.finding_id, args.status, args.description);
                }
                else if (action === "resolve") {
                    if (!args.description)
                        return "`description` is required to resolve a finding.";
                    resolveFinding(graph, {
                    ...
                    createFindingTask(graph, finding, {
                        purpose: purpose,
        ...
        "sdd.reverse_engineer": tool({
            ...
            async execute(args, ctx) {
                ...
                const findingSummary = materializeBrownfieldFindings(graph, result.findings, args.purpose);
                ...
                if (args.purpose === "reverse_engineering") {
                    ...
                    lines.push("", "### Next Steps", "This is a **technology-agnostic** SDD. When using it in a new project:", "1. The LLM will detect `purpose: reverse_engineering` in the graph", "2. It will ask you to choose a tech stack (frontend, backend, database, etc.)", "3. Architecture components will be updated with your chosen technologies", `4. Reverse-engineering findings converted to target requirements: ${findingSummary.resolvedForTarget}`, `5. Unresolved target gaps: ${unresolved.length}`, "6. Then proceed with the normal SDD workflow");
                }
                return lines.join("\n");
            },
        }),
        "sdd.generate_cicd": tool({
            description: "Generate CI/CD configuration files for the project. Supports GitHub Actions, GitLab CI, Jenkins, and Docker.",
            args: {
            ...
            async execute(args, ctx) {
                ...
                const change = requireApprovedWorkflowChange(ctx.directory, ctx.sessionID, graph);
                if (!change)
                    return "An approved Change is required before generating CI/CD files.";
                const config = {
        ...
        "sdd.bug_fix": tool({
            ...
            async execute(args, ctx) {
                if (!getRepo(ctx.directory).isInitialized())
                    return "SDD not initialized. Run sdd.initialize first.";
        ...
        "sdd.build_graph": tool({
            ...
            args: {
                ...
                analysis_json: tool.schema.string().optional().describe('Structured analysis JSON from LLM. Schema: { features: [{name, description, priority, phase?}], ' +
            ...
            async execute(args, ctx) {
                ...
                if (args.analysis_json) {
                    try {
                        ...
                        if (!analysis.endpoints || !Array.isArray(analysis.endpoints))
                            throw new Error("Missing or invalid 'endpoints' array");
                        if (!analysis.businessRules || !Array.isArray(analysis.businessRules))
                            throw new Error("Missing or invalid 'businessRules' array");
                        if (!analysis.architectureComponents || !Array.isArray(analysis.architectureComponents))
                            throw new Error("Missing or invalid 'architectureComponents' array");
        ...
        "sdd.drift_signals": tool({
            description: "Detect advanced drift signals: mutant duplicates (near-identical files), " +
        ...
        "sdd.migrate_storage": tool({
            description: "Migrate SDD storage between YAML and SQLite backends. " +
            ...
            async execute(args, ctx) {
                ...
                const graph = repo.loadGraph();
                repo.migrateTo(args.target, ctx.directory);
        ...
        "sdd.auto_link_tests": tool({
            description: "Automatically link orphan tests to requirements based on name matching, " +
            ...
            async execute(args, ctx) {
                ...
                for (const orphan of linkableOrphans) {
                    ...
                    const exists = graph.relationships.some(r => r.from === orphan.inferred_requirement_id && r.to === orphan.test_id && r.type === "tested_by");
                    if (exists)
                        continue;
                    try {
        ...
        "sdd.infer_relationships": tool({
            description: "Reconstrói a rastreabilidade do Knowledge Graph: infere arestas que a " +
            ...
            async execute(args, ctx) {
                ...
                if (!repo.isInitialized())
                    return "SDD not initialized.";
                const graph = repo.loadGraph();
                const { inferRelationships, runRelationshipInference } = await import("../sdd/discovery/relationship-inferencer.js");
                ...
                if (args.dry_run) {
                    ...
                    for (const proposal of proposals.slice(0, 25)) {
                        lines.push(`- ${proposal.from} --[${proposal.type}]--> ${proposal.to} ` +
                            `(${proposal.method}, conf ${proposal.confidence.toFixed(2)})`);
                    }
                    if (proposals.length > 25)
                        lines.push(`- ... e mais ${proposals.length - 25}`);
        ...
        "sdd.milestone": tool({
            description: "Gerencia milestones (âncora de release) e gera o relatório de rastreabilidade por release. " +
            ...
            async execute(args, ctx) {
                ...
                if (!repo.isInitialized())
                    return "SDD not initialized.";
                const graph = repo.loadGraph();
                const { createMilestone, linkNodesToMilestone, unlinkNodesFromMilestone, moveNodesToMilestone, closeMilestone, getMilestoneNodes, buildReleaseReport, formatReleaseReport, } = await import("../sdd/release/milestone.js");
                const action = args.action ?? "list";
                const parseIds = (value) => (value ?? "").split(",").map((v) => v.trim()).filter((v) => v.length > 0);
                const nameOf = (id) => graph.nodes.find((n) => n.id === id)?.name ?? id;
                try {
                    if (action === "create") {
                        if (!args.name)
                            return "`name` é obrigatório para create.";
                    ...
                    if (action === "list") {
                        ...
                        if (milestones.length === 0)
                            return "Nenhum milestone definido. Use action=\"create\".";
                    ...
                    if (action === "add" || action === "remove") {
                        if (!args.milestone_id)
                            return "`milestone_id` é obrigatório.";
                        ...
                        return `## Milestone remove\n**Desvinculados:** ${result.removed}`;
                    }
                    if (action === "assign") {
                        if (!args.from_id || !args.to_id)
                            return "`from_id` e `to_id` são obrigatórios.";
                        const ids = parseIds(args.node_ids);
                        if (ids.length === 0)
                            return "`node_ids` é obrigatório.";
                    ...
                    if (action === "close") {
                        if (!args.milestone_id)
                            return "`milestone_id` é obrigatório.";
                        const milestone = closeMilestone(graph, args.milestone_id, args.status ?? "COMPLETED");
                        repo.saveGraph(graph);
        ...
        "sdd.integrate_tasks": tool({
            description: "Kanban task board bridge: list tasks pending AI integration, create/update/remove tasks, " +
            ...
            args: {
                action: tool.schema
                    ...
                    .optional()
                    .describe("Operação: list (default) | create | update | remove | mark_integrated | open_change | approve_change"),
                ...
                no_requirement_impact: tool.schema
                    .boolean()
                    .optional()
                    .describe("open_change/approve_change: declara que o Change não altera comportamento especificado (pula a evidência de requisito)"),
            },
            async execute(args, ctx) {
                const repo = getRepo(ctx.directory);
                if (!repo.isInitialized())
                ...
                const formatTaskList = () => {
                    ...
                    if (tasks.length === 0)
                        return "## SDD Tasks — nenhuma task no board.";
                ...
                if (action === "create") {
                    if (!args.name)
                        return "`name` is required to create a task.";
                ...
                if (action === "update") {
                    if (!args.task_id)
                        return "`task_id` is required to update a task.";
                    const column = isTaskColumn(args.column) ? args.column : undefined;
                ...
                if (action === "remove") {
                    if (!args.task_id)
                        return "`task_id` is required to remove a task.";
                ...
                if (action === "mark_integrated") {
                    if (!args.task_id)
                        return "`task_id` is required to mark a task as integrated.";
                ...
                if (action === "open_change" || action === "approve_change") {
                    if (!args.task_id)
                        return "`task_id` is required to open the SDD Change of a task.";
