...
export function createSddTools() {
    const tools = {
        ...
        "sdd.add_node": tool({
            description: "Add a new node to the SDD Knowledge Graph. Supports all node types: " +
            ...
            async execute(args, ctx) {
                ...
                const existingByName = graph.nodes.find((n) => n.type === args.type && n.name.toLowerCase() === args.name.toLowerCase());
                if (existingByName) {
                    return [
        ...
        "sdd.add_relationship": tool({
            description: "Add a relationship between two nodes in the SDD Knowledge Graph. Prevents cycles and self-loops.",
            args: {
        ...
        "sdd.list_nodes": tool({
            ...
            async execute(args, ctx) {
                ...
                if (!repo.isInitialized())
                    return "SDD not initialized.";
                const indices = repo.getIndices();
                const nodes = getNodesByTypeIndexed(indices, args.type);
                if (nodes.length === 0)
                    return `No nodes of type "${args.type}" found.`;
        ...
        "sdd.find_path": tool({
            ...
            async execute(args, ctx) {
                ...
                if (!path)
                    return `No path found between ${args.from_id} and ${args.to_id}.`;
        ...
        "sdd.config_drift": tool({
            description: "Detect configuration drift: version mismatches, deprecated settings, " +
        ...
        "sdd.workflow_export": tool({
            description: "Export the current SDD workflow state as a structured report. " +
        ...
        "sdd.sync_pull": tool({
            ...
            async execute(_args, ctx) {
                ...
                if (!acquired)
                    return "Error: Another sync operation is in progress. Wait or clear the lock.";
                try {
        ...
        "sdd.set_role": tool({
            ...
            async execute(args, ctx) {
                setRole(ctx.directory, args.user, args.role);
        ...
        "sdd.check_permission": tool({
            ...
            async execute(args, ctx) {
                ...
                const allowed = checkPermission(role, args.permission, ctx.directory);
                addAuditEntry(ctx.directory, args.user, "check_permission", args.permission, allowed ? "allowed" : "denied");
                return formatPermissionCheck(args.user, role, args.permission, allowed);
            },
        }),
        "sdd.audit_log": tool({
        ...
        "sdd.generate_dashboard": tool({
            ...
            async execute(args, ctx) {
                ...
                const config = generateDashboardConfig(graph, args.type);
            ...
            async execute(_args, ctx) {
                ...
                function scanDir(dir, relativePath = "") {
                    ...
                    for (const entry of entries) {
                        if (["node_modules", ".sdd", "dist", "build", ".git", ".opencode"].includes(entry))
                            continue;
                        ...
                        try {
                            ...
                            if (stat.isDirectory()) {
                                ...
                                try {
                                    ...
                                    sddDebug("tools", `Failed to read ${fullPath} for usage analysis`);
                                }
                            }
                        }
                        catch (error) {
                            sddDebug("tools", `Failed to scan directory ${fullPath}`);
                        }
                    }
                }
        ...
        "sdd.remove_dead_code": tool({
            ...
            async execute(args, ctx) {
                ...
                const affectedNodes = [];
                const fileNodeId = graph.nodes.find(n => n.type === "file" && n.metadata.path === args.file)?.id;
                if (fileNodeId) {
        ...
        "sdd.remove_relationship": tool({
            ...
            async execute(args, ctx) {
                ...
                if (!repo.isInitialized())
                    return "SDD not initialized.";
                const graph = repo.loadGraph();
                removeRelationship(graph, args.from_id, args.to_id, args.type);
                repo.saveGraph(graph);
                invalidateCacheForMutation(ctx.directory, [], [args.type]);
        ...
        "sdd.traverse_outgoing": tool({
            ...
            async execute(args, ctx) {
                ...
                for (const [id, dist] of result.distances) {
                    ...
                    if (n) {
                        lines.push(`- **${id}** (depth ${dist}): ${n.name} (${n.type})`);
                        shown++;
                    }
                }
                return lines.join("\n");
        ...
        "sdd.get_nodes_by_status": tool({
            ...
            async execute(args, ctx) {
                ...
                if (nodes.length === 0)
                    return `No nodes with status "${args.status}".`;
                const { page, total, hasMore } = paginate(nodes);
        ...
        "sdd.merge_graphs": tool({
            ...
            async execute(args, ctx) {
                ...
                try {
                    ...
                    const remoteGraph = readJson(remoteGraphPath);
        ...
        "sdd.graph_health": tool({
            description: "Analyze graph health using indices: orphan nodes, disconnected components, " +
            ...
            async execute(_args, ctx) {
                ...
                if (sortedTypes.length > 10)
                    lines.push(`- ... and ${sortedTypes.length - 10} more types`);
                lines.push("");
        ...
        "sdd.graph_health_detail": tool({
            description: "Detailed graph health analysis using computeGraphHealth: stale changes, cycles, " +
            ...
            async execute(_args, ctx) {
                ...
                if (health.staleChangeIds.length > 0) {
                    ...
                    if (health.staleChangeIds.length > 10)
                        lines.push(`- ... and ${health.staleChangeIds.length - 10} more`);
                }
                else {
                ...
                if (health.godNodes.length > 0) {
                    ...
                    if (health.godNodes.length > 5)
                        lines.push(`- ... and ${health.godNodes.length - 5} more`);
                }
                else {
        ...
        "sdd.graph_prune": tool({
            description: "Prune obsolete nodes from the SDD graph: dead file references, orphaned symbols, " +
        ...
        "sdd.detect_conventions": tool({
            description: "Detect project coding conventions (naming, imports, async patterns, structure) " +
        ...
        "sdd.learn_patterns": tool({
            description: "Analyze the SDD graph to learn project patterns: common node defaults, " +
            ...
            async execute(args, ctx) {
                ...
                if (args.action === "show") {
                    ...
                    if (!patterns)
                        return "No patterns learned yet. Run with action=learn first.";
                    return formatPatterns(patterns);
                }
                if (args.action === "suggest") {
                    if (!args.node_type)
                        return "node_type is required for suggest action.";
                    const patterns = loadPatterns(ctx.directory);
                    if (!patterns)
                        return "No patterns learned yet. Run with action=learn first.";
                    const defaults = getSuggestedDefaults(patterns, args.node_type);
                    const rels = getSuggestedRelationships(patterns, args.node_type);
                    const naming = getSuggestedNaming(patterns, args.node_type);
                    const lines = [
        ...
        "sdd.cache_stats": tool({
            description: "Show cache statistics: hit rates, sizes, invalidation count. " +
        ...
        "sdd.whitelist_drift": tool({
            description: "Add a file or pattern to the drift whitelist. Whitelisted files will NOT be reported as drift. " +
        ...
        "sdd.list_whitelist": tool({
            ...
            async execute(_args, ctx) {
                ...
                for (const entry of whitelist.entries) {
                    ...
                    if (entry.added_by)
                        lines.push(`  Added by: ${entry.added_by}`);
