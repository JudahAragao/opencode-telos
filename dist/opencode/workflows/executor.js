 ...
 */
export async function executeChain(chain, initialParams, executeTool, config = DEFAULT_EXECUTOR_CONFIG, hooks) {
    ...
    for (let i = 0; i < chain.steps.length; i++) {
        ...
        try {
            ...
            const result = await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error(`Step timed out after ${config.stepTimeoutMs}ms`)), config.stepTimeoutMs)),
            ]);
            const stepResult = {
                stepIndex: i,
                tool: step.tool,
