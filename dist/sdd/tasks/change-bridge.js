...
export function findTaskChange(graph, taskId) {
...
function specTargets(graph, taskId) {
    ...
    for (const rel of graph.relationships) {
        ...
        const otherId = rel.from === taskId ? rel.to : rel.from;
        const other = getNode(graph, otherId);
        if (other && SPEC_NODE_TYPES.has(other.type))
            ids.add(other.id);
    }
    return [...ids];
}
function toResult(graph, change, created, extraWarnings = []) {
    const current = getNode(graph, change.id) ?? change;
    ...
    if (!approved) {
        ...
        if (current.metadata.approval_level === "BLOCKED") {
            ...
            blockers.push(`Change ${current.id} requires explicit approval (level: ${current.metadata.approval_level}). ` +
 ...
 */
export function openChangeForTask(graph, taskId, input = {}) {
    const task = getTask(graph, taskId);
    if (!task)
    ...
    if (!hasRequirement && affectedNodes.length > 0) {
        extraWarnings.push("Nenhum `requirement` está vinculado à task, então a evidência funcional do Change não é avaliável. " +
...
function syncTaskWithChange(graph, task, changeId) {
 ...
 */
export function buildChangeImplementationPrompt(change, task) {
...
export function formatOpenChangeResult(result) {
    const lines = [
...
export function getTasksAwaitingChangeApproval(graph) {
    const pending = [];
    for (const node of graph.nodes) {
        if (node.type !== "task")
            continue;
        const declared = node.metadata.change_id;
        ...
        if (change.status === "DRAFT" || change.status === "PROPOSED") {
            pending.push({ task: node, change: change });
        }
    }
    return pending;
}
