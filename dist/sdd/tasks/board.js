...
export const TASK_COLUMN_LABELS = {
    backlog: "Backlog",
    ready: "Ready",
...
export function isTaskColumn(value) {
    return typeof value === "string" && TASK_COLUMNS.includes(value);
}
...
export function getTaskNodes(graph) {
    return graph.nodes.filter((n) => n.type === "task");
}
export function getTask(graph, id) {
    const node = getNode(graph, id);
    return node && node.type === "task" ? node : undefined;
}
/**
 ...
 */
export function nextTaskId(graph) {
...
export function taskIntegrationStatus(task) {
    const value = task.metadata.integration_status;
    return value === "pending" || value === "integrated" ? value : "manual";
}
export function toTaskBoardItem(graph, task) {
    const links = [];
    for (const rel of graph.relationships) {
        if (rel.from !== task.id && rel.to !== task.id)
            continue;
...
function linkTask(graph, task, linkTo, linkType) {
    if (linkTo) {
        ...
        if (target) {
            const type = linkType ?? (target.type === "project" ? "contains" : "implements");
            if (type === "contains") {
...
export function createTask(graph, input) {
...
export function updateTask(graph, id, input) {
    ...
    if (input.expected_version !== undefined && task.version !== input.expected_version) {
        throw new Error(`Version conflict on ${id}: expected ${input.expected_version}, found ${task.version}`);
    }
    const updates = {};
    if (input.name !== undefined) {
        ...
        const duplicate = getTaskNodes(graph).find((t) => t.id !== id && t.name.toLowerCase() === name.toLowerCase());
        if (duplicate)
            throw new Error(`Task "${name}" already exists (${duplicate.id})`);
        updates.name = name;
    }
...
export function removeTask(graph, id) {
    const task = getTask(graph, id);
    if (!task)
        throw new Error(`Task ${id} not found`);
    removeNode(graph, id);
}
export function markTaskIntegrated(graph, id) {
    const task = getTask(graph, id);
    if (!task)
        throw new Error(`Task ${id} not found`);
    const metadata = {
    ...
    return updateNode(graph, id, { metadata });
}
export function getPendingIntegrationTasks(graph) {
 ...
 */
export function buildIntegrationBrief(graph) {
    ...
    for (const task of pending) {
        ...
        if (meta.goal)
            lines.push(`- Objetivo: ${String(meta.goal)}`);
    ...
    lines.push("**Como integrar:** vincule a task ao `feature`/`requirement` correspondente com `sdd.graph_mutation(action=\"add_relationship\")` " +
...
export function buildTaskIntegrationPrompt(taskId, name) {
    return [
