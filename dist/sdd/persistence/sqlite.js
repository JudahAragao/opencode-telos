...
export class SqliteGraphRepository {
    ...
    loadGraph() {
        ...
        // Load all nodes
        const nodeRows = db.query("SELECT * FROM nodes").all();
        ...
        const relRows = db.query("SELECT * FROM relationships").all();
        ...
        try {
            ...
            if (tamperResult.tampered) {
                ;
                graph.metadata.__tamper_warning = tamperResult.reason;
            }
        }
        catch {
            ;
            graph.metadata.__tamper_warning = "Graph integrity validation could not be completed";
        }
        return structuredClone(graph);
    }
    saveGraph(graph) {
        ...
        try {
            ...
            try {
                ...
                const upsertMeta = db.prepare("INSERT INTO graph_metadata (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value");
      ...
      `);
                for (const node of graph.nodes) {
                    upsertNode.run(node.id, node.type, node.name, node.status, node.version, node.description || null, JSON.stringify(node.metadata), node.created_at, node.updated_at, node.created_by || null, node.change_id || null, node.previous_version || null);
                }
      ...
      `);
                for (const rel of graph.relationships) {
                    upsertRel.run(rel.id, rel.from, rel.to, rel.type, JSON.stringify(rel.metadata));
                }
    ...
    getIndices() {
        const cached = SqliteGraphRepository.cache.get(this.dbPath);
        if (cached)
            return GraphIndices.from(structuredClone(cached.graph));
    ...
    getNodeCount() {
        const db = this.getDb();
        const result = db.query("SELECT COUNT(*) as count FROM nodes").get();
        return result?.count || 0;
    }
    getRelationshipCount() {
        const db = this.getDb();
        const result = db.query("SELECT COUNT(*) as count FROM relationships").get();
        return result?.count || 0;
    }
    getNodesByType(type) {
        // Check per-type cache first
        const cacheMgr = getCacheManager(require("path").dirname(this.baseDir));
        const currentFingerprint = (() => { try {
            return graphFingerprint(this.loadGraph());
        }
        catch {
            return "";
        } })();
        ...
        if (cached)
            return cached;
        const db = this.getDb();
        const rows = db.query("SELECT * FROM nodes WHERE type = ?").all(type);
    ...
    getNodesByStatus(status) {
        const db = this.getDb();
        const rows = db.query("SELECT * FROM nodes WHERE status = ?").all(status);
        return rows.map(row => ({
            id: row.id,
            type: row.type,
            name: row.name,
    ...
    getNodeById(id) {
        const db = this.getDb();
        const row = db.query("SELECT * FROM nodes WHERE id = ?").get(id);
        if (!row)
            return undefined;
        return {
            id: row.id,
    ...
    getRelationshipsForNode(nodeId) {
        const db = this.getDb();
        const rows = db.query("SELECT * FROM relationships WHERE from_id = ? OR to_id = ?").all(nodeId, nodeId);
        return rows.map(row => ({
            id: row.id,
            from: row.from_id,
            to: row.to_id,
    ...
    getGraphCounts() {
        const db = this.getDb();
        const nodeRow = db.query("SELECT COUNT(*) as c FROM nodes").get();
        const relRow = db.query("SELECT COUNT(*) as c FROM relationships").get();
        return {
    ...
    getMetadata() {
        const db = this.getDb();
        const rows = db.query("SELECT key, value FROM graph_metadata").all();
        const map = {};
     ...
     */
    getNodesSummary(offset, limit, type) {
        const db = this.getDb();
        ...
        const totalRow = db.query(countSql).get(...params);
        const total = totalRow?.c || 0;
        dataSql += " ORDER BY type, name LIMIT ? OFFSET ?";
        const rows = db.query(dataSql).all(...params, limit, offset);
        return {
            nodes: rows.map(row => ({
                id: row.id,
                type: row.type,
     ...
     */
    getRelationshipsSummary(offset, limit) {
        const db = this.getDb();
        const totalRow = db.query("SELECT COUNT(*) as c FROM relationships").get();
        const total = totalRow?.c || 0;
        const rows = db.query("SELECT id, from_id, to_id, type FROM relationships LIMIT ? OFFSET ?").all(limit, offset);
        return {
    ...
    getNodeTypeCounts() {
        const db = this.getDb();
        const rows = db.query("SELECT type, COUNT(*) as c FROM nodes GROUP BY type ORDER BY c DESC").all();
    ...
    getStatusCounts() {
        const db = this.getDb();
        const rows = db.query("SELECT status, COUNT(*) as c FROM nodes GROUP BY status ORDER BY c DESC").all();
    ...
    getAllNodesSummary() {
        const db = this.getDb();
        const rows = db.query("SELECT id, type, name, status, version FROM nodes ORDER BY type, name").all();
        return rows.map(row => ({
            id: row.id,
            type: row.type,
            name: row.name,
     ...
     * Returns just the dashboard-relevant fields.
     */
    getAllRelationshipsSummary() {
        const db = this.getDb();
        const rows = db.query("SELECT id, from_id, to_id, type FROM relationships").all();
        return rows.map(row => ({
            id: row.id,
            from: row.from_id,
            to: row.to_id,
    ...
    getUpdatedAt() {
        const db = this.getDb();
        const row = db.query("SELECT value FROM graph_metadata WHERE key = 'updated_at'").get();
        return row?.value || "";
    }
}
