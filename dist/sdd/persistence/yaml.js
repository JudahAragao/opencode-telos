 ...
 */
export class YamlGraphRepository {
    baseDir;
    graphPath;
    ...
    loadGraph() {
        ...
        const graph = readYaml(this.graphPath);
        const indices = GraphIndices.from(graph);
        const stat = (() => { try {
            return require("fs").statSync(this.graphPath).mtimeMs;
        }
        catch {
            return 0;
        } })();
    ...
    getNodeCount() {
        const cached = YamlGraphRepository.cache.get(this.graphPath);
        if (cached)
            return cached.indices.totalNodes;
    ...
    getRelationshipCount() {
        const cached = YamlGraphRepository.cache.get(this.graphPath);
        if (cached)
            return cached.indices.totalRelationships;
        return 0;
    }
    getNodesByType(type) {
