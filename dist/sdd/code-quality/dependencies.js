...
function calculateCoupling(nodes) {
    const afferent = {};
    const efferent = {};
    const instability = {};
    for (const node of nodes) {
...
export function analyzeDependencies(graph) {
    ...
    const mostCoupled = nodes.reduce((most, n) => (n.dependencies.length + n.dependents.length) > (most.dependencies.length + most.dependents.length) ? n : most, nodes[0]);
...
export function formatDependencyReport(report) {
    ...
    lines.push('', '### Recomendações', '- Evite ciclos de dependência', '- Mantenha a instabilidade baixa para módulos reutilizáveis', '- Use inversão de dependência para desacoplar módulos', '- Princípio Dependency Inversion (SOLID)');
    return lines.join('\n');
}
