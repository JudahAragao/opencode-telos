...
function createValidationCache() {
    ...
    return {
        get(graphVersion) {
            if (cache && cache.graphVersion === graphVersion)
                return cache;
            return null;
        },
        set(result, graphVersion, nodeCount) {
            version++;
 ...
 */
export function validateGraph(graph, policy = DEFAULT_VALIDATION_POLICY, projectDir) {
    const errors = [];
    const warnings = [];
    ...
    try {
        ...
        const tamperWarning = graph.metadata.__tamper_warning;
        if (tamperWarning) {
            errors.push({
                code: "GRAPH_TAMPERED",
        ...
        sddDebug("validator", "Legacy tamper check failed", error);
    }
    if (projectDir) {
...
function validateStructural(graph, errors, warnings) {
    const ids = new Set();
...
function validateSemantic(graph, errors, warnings, policy = DEFAULT_VALIDATION_POLICY) {
    ...
    for (const req of requirements) {
        ...
        const hasTestEvidence = graph.relationships.some((r) => r.from === req.id && (r.type === "tested_by" || r.type === "tests"));
        if (priority === "critical" && !hasTestEvidence) {
            ...
            if (policy.critical_requirement_without_test === "error")
                errors.push(issue);
            else
                warnings.push(issue);
        }
        ...
        if (!Array.isArray(verification?.scenarios) || verification.scenarios.length === 0) {
            ...
            if (policy.missing_verification_scenario === "error")
                errors.push(issue);
            else
                warnings.push(issue);
        }
    ...
    for (const entity of entities) {
        if (isNodeExcludedOrDeprecated(entity.id, entity.status, removed, deprecated))
            continue;
...
function validateReferences(graph, errors, _warnings) {
...
function validateCompleteness(graph, errors, warnings) {
 ...
 */
function validateTraceability(graph, warnings) {
    ...
    const linksFeature = (nodeId) => graph.relationships.some((r) => r.from === nodeId && featureIds.has(r.to) &&
    ...
    for (const endpoint of graph.nodes.filter((n) => n.type === "endpoint")) {
        ...
        const operatesOnEntity = graph.relationships.some((r) => r.from === endpoint.id &&
    ...
    if (hasFeature) {
        for (const req of graph.nodes.filter((n) => n.type === "requirement")) {
            const specifies = graph.relationships.some((r) => r.from === req.id && ["specifies", "traces_to"].includes(r.type));
            if (!specifies) {
                warnings.push({
    ...
    for (const task of graph.nodes.filter((n) => n.type === "task")) {
        const declared = task.metadata.change_id;
    ...
    for (const task of graph.nodes.filter((n) => n.type === "task")) {
        const linkedSpec = graph.relationships.some((r) => r.from === task.id && ["implements", "depends_on", "requires", "traces_to"].includes(r.type) &&
    ...
    for (const change of graph.nodes.filter((n) => n.type === "change")) {
        ...
        const files = Array.isArray(metadata.affected_files) ? metadata.affected_files : [];
        for (const path of files) {
            const fileId = graph.nodes.find((node) => node.type === "file" && (node.metadata.path === path || node.name === path))?.id;
    ...
    for (const requirement of graph.nodes.filter((n) => n.type === "requirement")) {
        const promises = requirement.metadata.promise_states;
        if (!promises)
            continue;
        for (const [promiseId, state] of Object.entries(promises)) {
 ...
 */
function validateCrossLayerDependencies(graph, _errors, warnings) {
    ...
    const fileDeps = graph.relationships.filter((r) => r.type === "uses" || r.type === "depends_on");
    const LAYER_ORDER = {
        shared: 0,
        database: 1,
        backend: 2,
    ...
    for (const dep of fileDeps) {
        ...
        if (!fromLayer || !toLayer || fromLayer === toLayer || fromLayer === "shared" || toLayer === "shared")
            continue;
    ...
    const symbolDeps = graph.relationships.filter((r) => r.type === "calls" || r.type === "uses");
    for (const dep of symbolDeps) {
        ...
        if (!fromSymbol || !toSymbol)
            continue;
        const fromMeta = fromSymbol.metadata;
        const toMeta = toSymbol.metadata;
        if (!fromMeta.file_path || !toMeta.file_path)
            continue;
 ...
 */
export function hasPersistenceMapping(entity, graph) {
    // Strategy 1: Explicit relationship
    const hasRel = graph.relationships.some((r) => r.from === entity.id && r.type === "persists_to");
    if (hasRel)
        return true;
 ...
 */
function validateSemanticDeep(graph, _errors, warnings) {
        ...
        for (const match of paramMatches) {
            ...
            if (relatedEntities.length > 0) {
                const hasField = relatedEntities.some(e => {
                    ...
                    return fields.some((f) => f.name === paramName);
                });
                if (!hasField) {
                    warnings.push({
    ...
    for (const feature of features) {
        const hasConnection = graph.relationships.some(r => (r.from === feature.id || r.to === feature.id) &&
    ...
    for (const rule of rules) {
        const constrainsSomething = graph.relationships.some(r => r.from === rule.id && r.type === "constrains");
...
function validateAcceptance(graph, errors, warnings) {
