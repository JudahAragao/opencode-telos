 ...
 */
export function getAffectedSubsystems(dirtyNodeIds, indices) {
 ...
 */
export function validateSmart(graph, dirtyNodeIds, options) {
    ...
    const { subsystems: autoDetectedSubsystems, nodeIds: relevantNodeIds } = getAffectedSubsystems(dirtyNodeIds, indices);
    // Apply options: merge additional, exclude specific, or validate all
    let subsystemsToCheck = new Set(autoDetectedSubsystems);
    ...
    const skippedSubsystems = [...allSubsystems].filter((s) => !subsystemsToCheck.has(s));
    ...
    const relevantRels = graph.relationships.filter((r) => relevantNodeIds.has(r.from) || relevantNodeIds.has(r.to));
...
function validateRequirementsSmart(indices, nodes, removed, deprecated, warnings) {
    ...
    for (const req of requirements) {
        ...
        const hasTask = outgoing.some((r) => r.type === "implemented_by" || r.type === "contains" || r.type === "belongs_to") || incoming.some((r) => r.type === "implements" && indices.byId.get(r.from)?.type === "task");
...
function validateEntitiesSmart(nodes, removed, deprecated, warnings) {
...
function validateEndpointsSmart(indices, nodes, removed, deprecated, warnings) {
    const endpoints = nodes.filter((n) => n.type === "endpoint");
    for (const ep of endpoints) {
        if (isNodeExcludedOrDeprecated(ep.id, ep.status, removed, deprecated))
            continue;
        const meta = ep.metadata;
        if (!meta.method)
            warnings.push({ code: "ENDPOINT_NO_METHOD", message: `Endpoint ${ep.id} has no HTTP method`, node_id: ep.id });
        if (!meta.path)
            warnings.push({ code: "ENDPOINT_NO_PATH", message: `Endpoint ${ep.id} has no path`, node_id: ep.id });
        // O(1) check using index
        const incoming = indices.getIncoming(ep.id);
        const hasApi = incoming.some((r) => r.type === "contains");
        if (!hasApi)
            warnings.push({ code: "ENDPOINT_ORPHAN", message: `Endpoint ${ep.id} is not contained by any API`, node_id: ep.id });
    }
}
function validateCrossLayerSmart(indices, nodes, relationships, warnings) {
    const LAYER_ORDER = {
    ...
    function inferLayer(path) {
        ...
        if (p.includes("client") || p.includes("frontend") || p.includes("pages/") || p.includes("components/"))
            return "frontend";
        if (p.includes("server") || p.includes("backend") || p.includes("routes/") || p.includes("services/"))
            return "backend";
    ...
    for (const f of fileNodes) {
        ...
        if (path) {
            ...
            if (layer)
                fileLayerMap.set(f.id, layer);
        }
    }
    for (const rel of relationships) {
        if (rel.type !== "uses" && rel.type !== "depends_on")
            continue;
...
function validateStructuralSmart(nodes, relationships, errors, warnings) {
    const ids = new Set();
    for (const rawNode of nodes) {
        ...
        ids.add(node.id);
        if (!node.type)
            errors.push({ code: "MISSING_TYPE", message: `Node ${node.id} missing type`, node_id: node.id });
        if (!node.name)
            warnings.push({ code: "MISSING_NAME", message: `Node ${node.id} missing name`, node_id: node.id });
        if (!node.status)
            warnings.push({ code: "MISSING_STATUS", message: `Node ${node.id} missing status`, node_id: node.id });
    }
    for (const rel of relationships) {
        if (!rel.from)
            errors.push({ code: "MISSING_RELATIONSHIP_FROM", message: `Relationship ${rel.id} missing 'from'`, relationship_id: rel.id });
        if (!rel.to)
            errors.push({ code: "MISSING_RELATIONSHIP_TO", message: `Relationship ${rel.id} missing 'to'`, relationship_id: rel.id });
        if (!rel.type)
            errors.push({ code: "MISSING_RELATIONSHIP_TYPE", message: `Relationship ${rel.id} missing type`, relationship_id: rel.id });
    }
}
function validateReferencesSmart(relationships, graph, errors) {
