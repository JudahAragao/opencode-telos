...
function neighbors(graph, id) {
    ...
    for (const rel of getOutgoing(graph, id)) {
        ...
        if (node)
            result.push({ node, rel, direction: "outgoing" });
    }
    for (const rel of getIncoming(graph, id)) {
        const node = getNode(graph, rel.from);
        if (node)
            result.push({ node, rel, direction: "incoming" });
    }
    return result;
}
export function analyzeNodeImpact(graph, nodeId, maxDepth = 5) {
    ...
    while (queue.length > 0) {
        ...
        for (const entry of neighbors(graph, current)) {
            ...
            if (!directionMap.has(entry.node.id))
                directionMap.set(entry.node.id, new Set());
    ...
    const build = (min, max) => [...distances.entries()]
    ...
    if (source.type === "requirement") {
        for (const rel of graph.relationships) {
            if (rel.from === source.id && rel.type === "has_acceptance_criterion")
...
export function formatNodeImpact(result) {
    const lines = [
    ...
    for (const group of ["direct", "indirect", "potential"]) {
