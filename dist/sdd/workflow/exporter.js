...
export function exportWorkflow(graph) {
    ...
    const changes = getNodesByType(graph, "change").map((c) => ({
        id: c.id,
        status: c.status,
        title: c.metadata.title,
        created_at: c.created_at,
    }));
    const decisions = getNodesByType(graph, "decision").map((d) => ({
        id: d.id,
    ...
    const blockers = graph.nodes
        ...
        node_type: n.type,
        name: n.name,
        reason: n.metadata.reason || "Unknown",
    }));
...
export function formatWorkflowExport(exp) {
    const lines = [
        `## SDD Workflow Export`,
