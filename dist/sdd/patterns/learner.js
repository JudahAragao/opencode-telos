 ...
 */
export function learnPatterns(graph) {
    const patterns = {
        metadata: {
...
function computeNodeDefaults(graph) {
    const defaults = {};
...
function computeRelationshipPatterns(graph) {
...
function computeApprovalHistory(graph) {
    const history = {};
    for (const node of graph.nodes) {
        if (node.type !== "change")
            continue;
...
function computeNamingConventions(graph) {
    const conventions = {};
    const byType = new Map();
    for (const node of graph.nodes) {
        const list = byType.get(node.type) || [];
    ...
    for (const [type, names] of byType) {
        ...
        if (total === 0)
            continue;
        if (pascalCase / total > 0.6)
            conventions[type] = "PascalCase";
        else if (camelCase / total > 0.6)
            conventions[type] = "camelCase";
        else if (snakeCase / total > 0.6)
            conventions[type] = "snake_case";
        else if (kebabCase / total > 0.6)
            conventions[type] = "kebab-case";
    }
    return conventions;
}
/**
 ...
 */
export function getSuggestedDefaults(patterns, nodeType) {
    return patterns.node_defaults[nodeType] || {};
}
/**
 * Get suggested relationships for a new node based on learned patterns.
 */
export function getSuggestedRelationships(patterns, nodeType) {
 ...
 */
export function getSuggestedNaming(patterns, nodeType) {
    return patterns.naming_conventions[nodeType];
}
/**
 * Load learned patterns from disk.
 */
export function loadPatterns(projectDir) {
 ...
 */
export function savePatterns(projectDir, patterns) {
    const path = join(projectDir, PATTERNS_FILE);
