...
export function detectAntiPatterns(graph) {
    const patterns = [];
...
function detectGodNodes(graph, patterns) {
    const nodeRelCounts = new Map();
...
function detectCircularDependencies(graph, patterns) {
    const adjList = new Map();
    for (const rel of graph.relationships) {
        if (!adjList.has(rel.from))
            adjList.set(rel.from, []);
...
function detectSpeculation(graph, patterns) {
...
function detectSkipSteps(graph, patterns) {
    const { removed, deprecated } = getExclusionSets(graph);
    const changes = getNodesByType(graph, "change");
    const nodesWithSpecs = new Set();
    for (const node of graph.nodes) {
        const specs = graph.relationships
            .filter((r) => r.from === node.id && r.type === "satisfied_by")
            .map((r) => r.to);
        if (specs.length > 0)
            nodesWithSpecs.add(node.id);
    }
    for (const node of graph.nodes) {
        if (isNodeExcludedOrDeprecated(node.id, node.status, removed, deprecated))
            continue;
        if (!nodesWithSpecs.has(node.id) && (node.type === "feature" || node.type === "requirement")) {
            const relatedChanges = changes.filter((c) => {
                return graph.relationships.some((r) => r.from === c.id && r.to === node.id && r.type === "modifies");
            });
...
function detectMissingTests(graph, patterns) {
    const { removed, deprecated } = getExclusionSets(graph);
    const requirements = getNodesByType(graph, "requirement")
        .filter((r) => !isNodeExcludedOrDeprecated(r.id, r.status, removed, deprecated));
    for (const req of requirements) {
        const hasTest = graph.relationships.some((r) => r.from === req.id && r.type === "tested_by");
        if (!hasTest) {
            patterns.push({
                type: "missing_test",
...
function detectEmptyNodes(graph, patterns) {
    const { removed, deprecated } = getExclusionSets(graph);
    for (const node of graph.nodes) {
        if (isNodeExcludedOrDeprecated(node.id, node.status, removed, deprecated))
            continue;
        const relCount = graph.relationships.filter((r) => r.from === node.id || r.to === node.id).length;
 ...
 */
function detectNearDuplicates(graph, patterns) {
    const { removed, deprecated } = getExclusionSets(graph);
    // Group nodes by type
    ...
    for (const node of graph.nodes) {
        if (isNodeExcludedOrDeprecated(node.id, node.status, removed, deprecated))
            continue;
        if (node.type === "project" || node.type === "change" || node.type === "symbol")
            continue;
...
export function formatAntiPatterns(result) {
    const lines = [
