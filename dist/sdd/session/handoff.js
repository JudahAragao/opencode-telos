...
export function generateHandoff(graph, projectDir, options) {
    ...
    const recentDecisions = getNodesByType(graph, "decision")
 ...
 */
function autoArchiveStaleChanges(graph) {
    const now = Date.now();
...
function buildSummary(graph, activeChanges, pendingPromises, qualityScore, blockedItems) {
    const parts = [];
...
export function computeGraphHealth(graph, options) {
    const now = Date.now();
    const STALE_DAYS = 7;
    const GOD_THRESHOLD = Math.max(50, graph.nodes.length * 0.25);
    // Stale changes (PROPOSED/APPROVED for >7 days)
    const staleChanges = [];
    const allChanges = graph.nodes.filter((n) => n.type === "change");
    ...
    let orphanChanges = 0;
    for (const change of allChanges) {
        const hasRels = graph.relationships.some((r) => r.from === change.id || r.to === change.id);
        if (!hasRels)
            orphanChanges++;
    }
    // DRAFT endpoints
    const draftEndpoints = graph.nodes.filter((n) => n.type === "endpoint" && n.status === "DRAFT").length;
    return {
...
export function formatHandoffPack(handoff) {
    const lines = [
        "## SDD Session Handoff",
    ...
    if (handoff.health) {
        lines.push("");
        lines.push(formatHealthReport(handoff.health));
    }
...
export function saveSessionLog(projectDir, action) {
