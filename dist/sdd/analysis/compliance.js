export function checkCompliance(_graph, standard, options) {
    const requirements = [];
    ...
    if (options?.customStandards?.length) {
        for (const custom of options.customStandards) {
            if (requirements.length >= maxReqs)
                break;
            if (options.excludeChecks?.includes(custom.id))
                continue;
            if (options.focusRequirements?.length && !options.focusRequirements.includes(custom.id))
                continue;
            requirements.push({
                id: custom.id,
    ...
    const filteredReqs = requirements.filter((r) => {
        if (options?.excludeChecks?.includes(r.id))
            return false;
        if (options?.focusRequirements?.length && !options.focusRequirements.includes(r.id))
            return false;
        return true;
    }).slice(0, maxReqs);
...
export function formatComplianceCheck(result) {
    const lines = [
