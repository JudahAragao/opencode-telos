...
export function calculateQualityScore(graph, projectDir, options) {
    const factors = [];
    ...
    const shouldInclude = (name) => {
        if (skipSet?.has(name))
            return false;
        if (focusSet && !focusSet.has(name))
            return false;
        return true;
    };
    const getCachedFactor = (name) => {
        ...
        if (Date.now() - cached.timestamp > maxAge)
            return null; // Expired
        return { name, score: cached.score, weight: FACTOR_WEIGHTS[name] || 0.1, details: cached.details };
    };
    const setCachedFactor = (name, score, details) => {
        if (options?.factorCache) {
...
function calculateTrend(history, _currentScore) {
    if (history.length < 3)
        return "stable";
