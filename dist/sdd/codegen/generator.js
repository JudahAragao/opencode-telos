...
export function detectTechStack(graph) {
    const archNodes = getNodesByType(graph, "architecture_component");
    const dbNodes = getNodesByType(graph, "database");
    const stack = {};
    for (const arch of archNodes) {
        ...
        if (layer === "frontend") {
            ...
            else if (tech.includes("svelte"))
                stack.frontend = "svelte";
            else
                stack.frontend = tech || undefined;
        }
        if (layer === "backend") {
            ...
            else if (tech.includes("nestjs"))
                stack.backend = "nestjs";
            else if (tech.includes("django"))
                stack.backend = "django";
            else if (tech.includes("fastapi"))
                stack.backend = "fastapi";
            else if (tech.includes("flask"))
                stack.backend = "flask";
            else if (tech.includes("rails"))
                stack.backend = "rails";
            else if (tech.includes("laravel"))
                stack.backend = "laravel";
            else if (tech.includes("gin"))
                stack.backend = "gin";
            else if (tech.includes("fiber"))
                stack.backend = "fiber";
            else if (tech.includes("actix") || tech.includes("axum"))
                stack.backend = "rust";
            else if (tech.includes("spring"))
                stack.backend = "spring";
            else
                stack.backend = tech || undefined;
        }
    }
    for (const db of dbNodes) {
        ...
        if (engine.includes("postgres"))
            ...
            stack.database = engine || undefined;
    }
    return stack;
}
...
export function hasCodegenSupport(stack) {
    return (!stack.backend || SUPPORTED_BACKENDS.has(stack.backend)) && (!stack.frontend || SUPPORTED_FRONTENDS.has(stack.frontend)) && (!stack.database || SUPPORTED_DATABASES.has(stack.database));
}
export function generateProject(graph, stack) {
    ...
    const endpoints = getNodesByType(graph, "endpoint")
        .filter((e) => !isNodeExcludedOrDeprecated(e.id, e.status, removed, deprecated));
    const tables = getNodesByType(graph, "table")
        .filter((t) => !isNodeExcludedOrDeprecated(t.id, t.status, removed, deprecated));
    const businessRules = getNodesByType(graph, "business_rule")
...
function generateSpecPrompt(graph, stack) {
    const entities = getNodesByType(graph, "entity");
    const endpoints = getNodesByType(graph, "endpoint");
    const businessRules = getNodesByType(graph, "business_rule");
    ...
    const principlesList = (() => {
        if (!constitution)
            return "";
        const meta = constitution.metadata;
        if (!meta.principles)
            return "";
        return meta.principles.map((p) => `- [${p.severity}] ${p.rule}`).join("\n");
    })();
    const prompt = [
    ...
    prompt.push(`### Instructions`, `Generate a complete project structure for a ${stackDesc} application.`, `Include:`, `1. Project configuration files (package.json, tsconfig, etc.)`, `2. Backend: routes, controllers/services, database access layer`, `3. Frontend: components, pages, state management`, `4. Database schema/migrations`, `5. Tests`, `6. README with setup instructions`, ``, `Follow the conventions and patterns commonly used with the specified technologies.`, `Use the entity fields and API endpoints above to generate the actual code.`);
    if (decisionsList) {
...
function generateExpressBackend(_graph, entities, _endpoints, _tables, _rules, files, dirs, _stack) {
    dirs.add("src/server");
...
function generateReactFrontend(graph, entities, _features, _endpoints, files, dirs, _stack) {
    dirs.add("src/client");
...
`,
        ...
        // List component
        const tableHeaders = fields.slice(0, 5).map(f => `              <th className="px-4 py-2 text-left text-sm font-medium text-gray-700">${f.name}</th>`).join("\n");
        const tableCells = fields.slice(0, 5).map(f => `              <td className="px-4 py-2 text-sm text-gray-600">{item.${f.name}}</td>`).join("\n");
        files.push({
...
`,
    ...
    // App.tsx
    const pageImports = entities.map(e => `import { ${e.name}Page } from "./pages/${e.name}Page.js"`).join("\n");
...
function generateDatabaseSchema(_graph, entities, _tables, files, dirs, stack) {
    dirs.add("src/server");
...
function generateTests(_graph, entities, _endpoints, files, dirs, _stack) {
    dirs.add("tests");
    for (const entity of entities) {
        const name = entity.name;
        const nameLower = name.toLowerCase();
...
function generateSharedTypes(_graph, entities, files, dirs, _stack) {
    dirs.add("src/shared");
...
function mapToTsType(fieldType) {
    ...
    if (lower === "string" || lower === "text" || lower === "varchar")
        return "string";
    if (lower === "number" || lower === "integer" || lower === "int" || lower === "float" || lower === "decimal")
        return "number";
...
function buildGenerationSummary(graph, entities, features, files, stack) {
    const lines = [
 ...
 */
export function writeGeneratedFiles(projectDir, plan, options = {}) {
    let written = 0;
    let created = 0;
    let unchanged = 0;
    const conflicts = [];
    ...
    for (const dir of plan.directories) {
        let fullPath;
        try {
            fullPath = projectPath(projectDir, dir, true);
        }
        catch (error) {
            errors.push(`Refusing to create directory outside target: ${dir}: ${error}`);
            continue;
        }
