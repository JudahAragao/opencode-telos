 ...
 */
export function pruneGraph(graph, projectDir) {
    const report = {
        removed_nodes: [],
        deprecated_changes: [],
        total_removed: 0,
...
function pruneDeadFileNodes(graph, projectDir, report) {
    ...
    for (const fileNode of fileNodes) {
        ...
        if (!existsSync(fullPath)) {
            ...
            try {
                ...
                sddDebug("pruner", `Failed to remove file node ${fileNode.id}`, error);
            }
        }
    }
}
function pruneDeadSymbolNodes(graph, projectDir, report) {
    const symbolNodes = getNodesByType(graph, "symbol");
    for (const symNode of symbolNodes) {
        ...
        if (!existsSync(fullPath)) {
            ...
            try {
                ...
                sddDebug("pruner", `Failed to remove symbol node ${symNode.id}`, error);
            }
            continue;
        }
        ...
        try {
            ...
            if (!content.includes(name)) {
                ...
                try {
                    ...
                    sddDebug("pruner", `Failed to remove dead symbol ${symNode.id}`, error);
                }
            }
        }
        catch (error) {
            sddDebug("pruner", `Failed to read file for symbol check: ${filePath}`, error);
        }
    }
}
function pruneDeadTestNodes(graph, projectDir, report) {
    const testNodes = getNodesByType(graph, "test");
    for (const testNode of testNodes) {
        const target = testNode.metadata.target;
        if (!target)
            continue;
        ...
        if (!existsSync(fullPath)) {
            ...
            try {
                ...
                sddDebug("pruner", `Failed to remove test node ${testNode.id}`, error);
            }
        }
    }
}
function archiveOldChanges(graph, report) {
    const now = Date.now();
    const ARCHIVE_DAYS = 30;
    const changeNodes = getNodesByType(graph, "change");
...
function pruneDuplicateRelationships(graph, report) {
    const seen = new Set();
    const toRemove = [];
