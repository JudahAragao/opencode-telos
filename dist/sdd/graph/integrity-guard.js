 ...
 */
function loadIntegrityState(projectDir) {
 ...
 */
function saveIntegrityState(projectDir, state) {
    const path = join(projectDir, INTEGRITY_FILE);
 ...
 */
export function recordLegitimateSave(projectDir, graph, changeId) {
 ...
 */
export function validateGraphIntegrity(projectDir, graph) {
    const state = loadIntegrityState(projectDir);
    ...
    if (isSuspicious) {
        ...
        return {
            valid: false,
            tampered: true,
            reason: `⚠️ TAMPER DETECTED: Graph was modified outside the SDD workflow.\n` +
 ...
 */
export function getTamperLog(projectDir) {
    const state = loadIntegrityState(projectDir);
 ...
 */
export function isGraphTampered(projectDir) {
    const state = loadIntegrityState(projectDir);
 ...
 */
export function clearTamperFlag(projectDir) {
    const state = loadIntegrityState(projectDir);
    state.tampered = false;
    state.tamperCount = 0;
    saveIntegrityState(projectDir, state);
 ...
 */
export function formatTamperReport(result) {
    if (!result.tampered) {
