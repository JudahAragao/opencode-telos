import { getNode, addRelationship, } from "./engine.js";
import { getInverseRelationshipType, inverseKeyOf, preferredInverseType, relationshipKey, } from "./schema.js";
// ─── Main Entry Point ──────────────────────────────────────────────
/**
 ...
 */
export function ensureGraphIntegrity(graph, options = {}) {
 ...
 */
function filterGraphToFocus(graph, focusIds) {
    const neighborIds = new Set();
    for (const rel of graph.relationships) {
        if (focusIds.has(rel.from))
            neighborIds.add(rel.to);
        if (focusIds.has(rel.to))
...
function detectRedundantRelationships(graph) {
    const redundants = [];
    const flagged = new Set();
    const seen = new Set();
    const byKey = new Map();
    const flag = (rel, reason) => {
        const key = relationshipKey(rel.from, rel.to, rel.type);
        if (flagged.has(`${key}::${reason}`))
            return;
...
function isBidirectionalType(type) {
...
function removeRedundantRelationship(graph, redundant) {
    // Find and remove the actual relationship object
    const idx = graph.relationships.findIndex((r) => r.from === redundant.from && r.to === redundant.to && r.type === redundant.type);
    if (idx !== -1) {
...
function detectOrphanNodes(graph) {
    const orphans = [];
...
function connectOrphanNode(graph, orphan) {
...
function detectDisconnectedGroups(graph) {
    const projectId = graph.project_id;
...
function findBestConnectionTarget(graph, group, mainGroup) {
...
function suggestRelationshipType(source, target) {
    const s = source.type;
    const t = target.type;
...
function connectDisconnectedGroup(graph, group, _mainGroup) {
    ...
    if (!getNode(graph, sourceNodeId) || !getNode(graph, targetId))
        return null;
    try {
