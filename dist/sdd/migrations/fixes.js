...
registerMigration({
    ...
    up: (projectDir) => {
        const yamlPath = join(projectDir, ".sdd", "graph.yaml");
        if (!existsSync(yamlPath))
            return { success: true, message: "No graph.yaml" };
        try {
            const { createRepository } = require("../../sdd/persistence/repository.js");
            ...
            if (!modified)
                return { success: true, message: "All nodes already have priority" };
...
registerMigration({
    ...
    up: (projectDir) => {
        ...
        if (!existsSync(whitelistPath))
            return { success: true, message: "No whitelist found" };
        try {
...
registerMigration({
    ...
    up: (projectDir) => {
        ...
        try {
            ...
            if (fixed === 0)
                return { success: true, message: "Graph structure is valid" };
            repo.saveGraph(graph);
...
registerMigration({
    ...
    up: (projectDir) => {
        try {
            ...
            const repo = createRepository(projectDir);
            if (!repo.isInitialized())
                return { success: true, message: "No graph found, skipping" };
            const graph = repo.loadGraph();
