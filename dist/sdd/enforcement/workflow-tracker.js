...
export function workflowScope(projectDir, sessionId) {
...
function getMutableState(scope = DEFAULT_SCOPE) {
...
export function resetWorkflowState(scope = DEFAULT_SCOPE) {
...
export function markEnforced(changeId, scope = DEFAULT_SCOPE) {
    const state = getMutableState(scope);
    state.enforced = true;
...
export function markApproved(scope = DEFAULT_SCOPE) {
    const state = getMutableState(scope);
    state.approved = true;
}
/** Mark that sdd.validate passed */
export function markValidated(scope = DEFAULT_SCOPE) {
    const state = getMutableState(scope);
    state.validated = true;
}
/** Mark that discovery is in progress */
export function markDiscovering(discovering, scope = DEFAULT_SCOPE) {
    const state = getMutableState(scope);
    state.discovering = discovering;
}
/** Mark that the SDD spec has been updated (add_node, update_node, update_from_answers) */
export function markSpecUpdated(scope = DEFAULT_SCOPE) {
    const state = getMutableState(scope);
    state.specUpdated = true;
}
/** Mark that the change was completed (resets state) */
export function markCompleted(scope = DEFAULT_SCOPE) {
    const state = getMutableState(scope);
    state.enforced = false;
    state.changeId = null;
    state.approved = false;
...
export function isWorkflowValid(scope = DEFAULT_SCOPE) {
    const state = getMutableState(scope);
    if (!state.enforced) {
        return {
            valid: false,
    ...
    if (Date.now() - state.enforcedAt > ttl) {
        return {
            valid: false,
            reason: `SDD workflow expired after ${Math.round(ttl / 60000)} min. ` +
...
export function workflowRemainingMs(scope = DEFAULT_SCOPE) {
    const state = getMutableState(scope);
    if (!state.enforced)
        return 0;
 ...
 */
export function renewWorkflow(changeId, scope = DEFAULT_SCOPE) {
    const state = getMutableState(scope);
...
export function getWorkflowState(scope = DEFAULT_SCOPE) {
    const exact = states.get(scope);
    if (exact)
        return { ...exact };
    if (scope !== DEFAULT_SCOPE) {
        const sessionState = [...states.entries()].find(([key, value]) => key.startsWith(`${scope}::session:`) && value.enforced)?.[1];
        if (sessionState)
            return { ...sessionState };
    }
    return { ...freshState() };
 ...
 */
export const CLASSIFIED_TOOLS = new Set([
 ...
 */
export function isToolClassified(toolName) {
...
const COMPOSITE_MUTATING_ACTIONS = {
 ...
 */
export function checkToolAccess(toolName, scope = DEFAULT_SCOPE, action) {
    // SDD tools only
