export function generateDocumentation(graph, config) {
    let document;
    if (config.type === 'api') {
...
function generateFindingsSection(graph) {
    ...
    for (const node of findings) {
        ...
        if (meta.expected_behavior)
            lines.push(`- Esperado: ${meta.expected_behavior}`);
        if (meta.resolution?.description)
            lines.push(`- Resolução: ${meta.resolution.description}`);
    }
    if (findings.length === 0)
        lines.push('', 'Nenhuma descoberta foi registrada.');
    return lines.join('\n');
}
function generateAPIDocumentation(graph) {
    const lines = [
        '# Documentação da API',
        '',
        '## Endpoints',
...
function generateUserGuide(graph) {
    const lines = [
        '# Guia do Usuário',
        '',
        '## Visão Geral',
...
function generateDeveloperGuide(graph) {
    const lines = [
    ...
    if (entities.length > 0) {
        ...
        for (const entity of entities) {
            ...
            lines.push(`- **${entity.name}**: ${fields.map((f) => f.name).join(', ')}`);
        }
    }
    lines.push('', '## Desenvolvimento', '1. Clone o repositório', '2. Instale as dependências', '3. Execute os testes', '4. Siga o workflow SDD');
    return lines.join('\n');
}
