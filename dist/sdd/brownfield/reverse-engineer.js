...
function parsePrismaSchema(content) {
    ...
    for (const block of modelBlocks) {
        ...
        for (const line of lines) {
            ...
            if (fieldName === "id" && fieldType === "String")
...
function mapPrismaType(type) {
    ...
    if (t === "string" || t === "text")
        return "string";
    if (t === "int" || t === "integer" || t === "float" || t === "decimal" || t === "bigint")
        return "integer";
    if (t === "boolean" || t === "bool")
        return "boolean";
...
function extractTsEntities(filePath, content) {
    const entities = [];
    ...
    while ((match = interfacePattern.exec(content)) !== null) {
        const name = match[1];
        // Skip internal types
        if (["Props", "State", "Config", "Options", "Result", "Response", "Request", "Context", "Event"].some(s => name.endsWith(s)))
            continue;
        const body = match[2];
        const fields = [];
    ...
    while ((match = classPattern.exec(content)) !== null) {
        const name = match[1];
        if (name.endsWith("Service") || name.endsWith("Controller") || name.endsWith("Handler") || name.endsWith("Test"))
            continue;
        const body = match[2];
        const fields = [];
...
function mapTsType(type) {
    ...
    if (t.includes("date"))
        return "timestamp";
    if (t.includes("json") || t.includes("record"))
        return "json";
    if (t.includes("uuid"))
        return "uuid";
...
function extractBusinessRules(filePath, content) {
    const rules = [];
    const fileName = basename(filePath);
...
function detectArchitecture(brownfield, projectDir) {
    const arch = [];
    ...
    // Backend detection
    const backendDirs = dirs.filter(d => /^(?:src[\\/])?(?:server|backend|api|routes|controllers|services|middleware)/i.test(d) ||
    ...
    const infraFiles = brownfield.config_files.filter(f => /(?:docker|k8s|terraform|nginx|ci|cd|deploy|workflow)/i.test(f));
...
// ── Main Scanner ───────────────────────────────────────────────────
export function reverseEngineerProject(projectDir, options) {
...
// ── Helpers ────────────────────────────────────────────────────────
function findFilesByPatterns(projectDir, patterns, options) {
    const results = [];
...
function inferDomains(entities, endpoints) {
    const domains = new Set();
    for (const e of entities) {
        const name = e.name.toLowerCase();
        if (name.includes("user") || name.includes("auth") || name.includes("role"))
            domains.add("security");
        if (name.includes("order") || name.includes("payment") || name.includes("invoice"))
            domains.add("commerce");
        if (name.includes("product") || name.includes("catalog") || name.includes("inventory"))
            domains.add("catalog");
        if (name.includes("post") || name.includes("article") || name.includes("content") || name.includes("page"))
            domains.add("content");
        if (name.includes("notification") || name.includes("email") || name.includes("message"))
            domains.add("communication");
        if (name.includes("file") || name.includes("media") || name.includes("upload"))
            domains.add("media");
        if (name.includes("log") || name.includes("audit") || name.includes("event"))
            domains.add("analytics");
        if (name.includes("tenant") || name.includes("organization") || name.includes("team"))
            domains.add("multi-tenancy");
    }
...
function buildSummary(entities, endpoints, rules, architecture, brownfield, purpose) {
    ...
    if (endpoints.length > 0) {
        ...
        if (endpoints.length > 15)
            lines.push(`- ... and ${endpoints.length - 15} more`);
    }
