...
export function analyzeBriefing(briefing, options) {
    ...
    if (shouldDetect("domain")) {
        const domainPatterns = [
            ...
            ...(options?.customDomains?.map(c => [c.name, new RegExp(c.pattern, 'i')]) || []),
        ];
...
function extractContextAround(text, index) {
...
export function generateDiscoveryQuestions(analysis, options) {
    ...
    const questions = analysis.missing_information
        .filter((m) => {
        if (m.already_answered)
            return false;
        if (!m.question_for_user)
            return false;
        if (excludeSet.has(m.category))
            return false;
        if (focusSet && !focusSet.has(m.category))
            return false;
        if (focusClassSet && !focusClassSet.has(m.classification))
            return false;
        // Skip if already asked
        const qLower = m.question_for_user.question.toLowerCase();
        if (askedSet.has(qLower))
            return false;
        return true;
    })
...
export function formatDiscoverySummary(analysis) {
    const lines = ["## Briefing Analysis\n"];
    // Tech stack detected
