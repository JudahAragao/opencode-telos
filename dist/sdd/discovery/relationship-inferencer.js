...
import { getCanonicalRelationshipType, getInverseRelationshipType, isRelationshipAllowed, preferredInverseType, relationshipKey, } from "../graph/schema.js";
...
function nameTokens(node) {
    return tokenize(node.name);
}
function pathTokens(node) {
 ...
 */
export function ensureMilestoneNodes(graph) {
    let created = 0;
    const milestoneCache = new Map();
    const findOrCreateMilestone = (name, releaseVersion) => {
        const cacheKey = `${name}::${releaseVersion ?? ""}`;
        const cached = milestoneCache.get(cacheKey);
        if (cached)
            return cached;
        const existing = graph.nodes.find((node) => node.type === "milestone" && node.name.toLowerCase() === name.toLowerCase());
        if (existing) {
            // Carimba a versão de release quando descoberta por metadata.release.
            if (releaseVersion && !existing.metadata.release_version) {
                const updates = {};
                updates.metadata = {
                    ...existing.metadata,
    ...
    for (const node of [...graph.nodes]) {
        ...
        if (milestoneId && getNode(graph, milestoneId))
            targetIds.add(milestoneId);
 ...
 */
export function inferRelationships(graph, options = {}) {
    ...
    const add = (proposal) => {
        ...
        if (!fromType || !toType)
            return;
        if (!isRelationshipAllowed(fromType, proposal.type, toType))
            return;
        ...
        if (inverse) {
            ...
            if (existing.has(inverseKey) && preferredInverseType(inverse, proposal.type) === "a")
                return;
        }
        const current = proposals.get(key);
        if (!current || proposal.confidence > current.confidence)
    ...
    const bestMatches = (source, sourceTokens, candidates, type, method, baseConfidence, minScore) => {
 ...
 */
export function applyInferredRelationships(graph, proposals) {
    let applied = 0;
    let skipped = 0;
    const byType = {};
    const existing = new Set(graph.relationships.map((rel) => relationshipKey(rel.from, rel.to, rel.type)));
 ...
 */
export function normalizeInverseRelationships(graph) {
    const byKey = new Map();
    ...
    for (const rel of graph.relationships) {
        ...
        if (!inverseRel || inverseRel === rel)
            continue;
        if (toRemove.has(rel.id) || toRemove.has(inverseRel.id))
            continue;
 ...
 */
export function runRelationshipInference(graph, options = {}) {
    let normalized = 0;
