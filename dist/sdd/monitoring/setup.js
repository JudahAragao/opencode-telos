export function setupMonitoring(graph) {
    const metrics = [];
    const alerts = [];
    const dashboards = [];
...
export function formatMonitoringSetup(setup) {
    const lines = [
...
export function generateDashboardConfig(graph, dashboardType) {
    const panels = [];
...
export function formatDashboardConfig(config) {
    const lines = [
        `## ${config.title}`,
        '',
    ...
    lines.push('### Configuração Grafana', '1. Importe este dashboard no Grafana', '2. Configure a fonte de dados (Prometheus)', '3. Ajuste os intervalos conforme necessário');
    return lines.join('\n');
}
